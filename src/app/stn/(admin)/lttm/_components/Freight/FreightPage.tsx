...
import { Label, RiceBowl } from '@mui/icons-material'
...
import { getCtpvCd, getExcelFile, getToday, getLocGovCd } from '@/utils/fsms/common/comm'
...
import CustomDialog from '@/app/components/popup/CustomDialog'
...
import CustomTableDataGrid from './CustomTableDataGrid2'
import CustomTableDataGrid2 from './CustomTableDataGrid2'
import { LoadingBackdrop } from '@/app/components/loading/LoadingBackdrop'
import { stnLttmLgovTrnsfrnRequstHc } from '@/utils/fsms/headCells'
...
const checkAllEffectiveness = (rows :Row[], userLocgov: string):boolean => {
    ...
    return rows.filter((row) => {
        return userLocgov === row.chgLocgovCd // 사용자 관할관청과 전출관청이 같은지 확인
        &&  row?.dmndSeCd === '01' && row?.prcsSttsCd  === '01'  ; // 요청수신인지 확인
    }).length == rows.length && rows.length > 0;
}

const checkEffectiveness = (row: Row, userLocgov: string ): boolean => {
    return (
      userLocgov === row.chgLocgovCd  // 사용자 관할관청과 전출관청이 같은지 확인
      &&  row?.dmndSeCd === '01' && row?.prcsSttsCd  === '01'           // 요청수신인지 확인
    );
};
...
const checkCalcelEffectiveness = (row: Row, userLocgov: string): boolean => {
    return (
      userLocgov === row.chgLocgovCd && // 사용자 관할관청과 전출관청이 같은지 확인
      row?.dmndSeCd === '02' && row?.prcsSttsCd === '01'          // 요청발신인지 확인 
    );
};
...
type listSearchObj = {
    ...
    const [totalRows, setTotalRows] = useState(0) // 총 수f
    const [loading, setLoading] = useState(false) // 로딩여부

    const [selectedIndex, setSelectedIndex] = useState<number>(-1)
    const [selectedRows, setSelectedRows] = useState<string[]>([]) // 체크 로우 데이터

    const [isCurrlocgov,setIsCurrLocgov] = useState<boolean>(false)
...
//  const { authInfo } = useContext(UserAuthContext)
 ...
 // Fetch를 통해 데이터 갱신
    ...
    let endpoint: string =
        `/fsm/stn/lttm/tr/getAllLgovTrnsfrnRequst?page=${params.page}&size=${params.size}` +
    ...
    const putDataList = async (row : any[]) => {
        ...
        // 검색 조건에 맞는 endpoint 생성
        let endpoint: string =
            `/fsm/stn/lttm/tr/updateLgovTrnsfrnRequestList`
    ...
    const handleConfirm = async () => {
        ...
        && selectedRow.dmndSeCd === '02') && (selectedRow.chgLocgovNm?.split(" ")[1] === locgovNm)) ){
    ...
    const buttonGroupActions: ButtonGroupActionProps = {
        ...
        onClickApporveAllBtn: async function (rows : Row[]): Promise<void> {
            ...
            if(!confirm('선택된 지자체이관전입을 일괄 승인 하시겠습니까?')){
                return
            }
            ...
            const invalidRows = rows.filter(row => row.dmndSeCd !== '01' && row.prcsSttsCd !== '01' );
        ...
        onClickDeclineAllBtn: async function (rows : Row[]): Promise<void> {
            ...
            if(!confirm('선택된 지자체이관전입을 일괄 거절 하시겠습니까?')){
                return
            }
    ...
    const excelDownload = async () => {
        ...
        if(rows.length === 0){
            alert('엑셀 다운로드 할 데이터가 없습니다.')
            return
        }   
        try{
            setLoadingBackdrop(true)
            let endpoint: string =
            `/fsm/stn/lttm/tr/getExcelLgovTrnsfrnRequst?` +
            `${params.ctpvCd ? '&ctpvCd=' + params.ctpvCd : ''}` +
            `${params.locgovCd ? '&locgovCd=' + params.locgovCd : ''}` +
            `${params.vhclNo ? '&vhclNo=' + params.vhclNo : ''}` +
    ...
    return (
        ...
        <LocHistoryModal 
            ...
            data={selectedRow as {}}
            type={'tr'}
            url={'/fsm/stn/lttm/tr/getAllLgovTrnsfrnHst'}
        />
        ...
        <Box>
        <CustomTableDataGrid
            headCells={stnLttmLgovTrnsfrnRequstHc} // 테이블 헤더 값
            rows={rows} // 목록 데이터
