...
const ModalCalendar = (props: ModalFormProps) => {
  ...
  const fetchData = async () => {
    try {
      let endpoint: string =
        `/fsm/stn/tdgm/tx/getAllTaxiDayoffCalList?page=0` +
        `${dayoffYm ? '&dayoffYm=' + dayoffYm : ''}` +
        `${data.dayoffLocgovCd ? '&locgovCd=' + data.dayoffLocgovCd : ''}` +
            ...
            <div className="modal_content">
              <div className="calendar_wrap">
                <div className="calendar_date">
                  <button
                    type="button"
                    className="calendar_date_btn prev"
                  ...
                  <button
                    type="button"
                    className="calendar_date_btn next"
                    onClick={() => handleDateChange(+1)}
                  >
                    다음 달
                  </button>
                        ...
                        <tr key={weekIndex}>
                          {week.map((day, dayIndex) => {
                            const dayData = calRow.find(
                              (row) =>
                                parseInt(row.dayoffYmd, 10) === day &&
                                parseInt(row.calYear, 10) ===
                                  parseInt(dayoffYm.substring(0, 4), 10) &&
                                parseInt(row.calMonth, 10) ===
                                  parseInt(dayoffYm.substring(4, 6), 10),
                            )
                            return (
                              <td
                                key={dayIndex}
                                //className={dayData ? 'today' : ''}
                              >
                                {day ? (
                                  <div>
