...
function FormModal(props: FormModalProps) {
  ...
  const createBsnsfrnRequ = async () => {
    try {
      let endpoint: string = `/fsm/stn/ltmm/bs/createLgovMvtRequst`
      ...
      const userConfirm: boolean = confirm('버스 차량 전출등록을 하시겠습니까?')
      ...
      if (userConfirm) {
        ...
        alert('차량 전출 등록 취소하셨습니다.')
        return
      }
    } catch (error) {
      alert('차량 전출 실패 Error')
  ...
  return (
    <Box>
              ...
              <table className="table table-bordered">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td>{locgov?.ctpvNm + ' ' + locgov?.locgovNm}</td>
                  </tr>
                </tbody>
              </table>
            </div>
      ...
      <VhclSearchModal
        ...
        title="차량번호 조회"
        url="/fsm/stn/ltmm/bs/getAllVhcleMng"
        open={vhclOpen}
        isNotRollCheck={true}
      />
    </Box>
