...
const CustomTableDataGrid: React.FC<ServerPaginationGridProps> = ({
  ...
  const handleSelectAll = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const eligibleRows = rows
      ...
      .filter(row => row.prcsSttsCd === '01' && row.dmndSeCd === '02')
      .map(row => row.key);
                  ...
                  {oneCheck ? null : (
                    <>
                      ...
                      <CustomCheckbox
                        id="all"
                        indeterminate={
                          selected.length > 0 && selected.length < rows.filter(row => row.prcsSttsCd === '01' && row.dmndSeCd === '02').length
                        }
                        checked={rows.filter(row => row.prcsSttsCd === '01' 
                          && row.dmndSeCd === '02').length!== 0 
                          && selected.length === rows.filter(row => row.prcsSttsCd === '01' 
                            && row.dmndSeCd === '02').length}
                            ...
                            {headCell.format === 'button' ? (
                              ...
                              <TableCell padding="checkbox">
                                ...
                                <CustomCheckbox
                                  ...
                                  onChange={() => {
                                    (row?.dmndSeCd === '02' && row?.prcsSttsCd === '01') &&  
                                    handleSelect('tr' + index, row, index)}
                                  } // row와 index 추가
                                  disabled={rows[index]?.dmndSeCd !== '02'} 
                                />
                              </TableCell>
                            ) : (headCell.rowspan &&
