...
const LawSection  = ({queryString, law, handleSelected}: Props) => {
    const searchCategory = useSelector((state: AppState) => state.searchCategory)

    return (
        <div className="search-section">
            <div className="section-title">
            <h1 className="page-title">소송/심판 사례<span className="result-total">[검색결과 {getCommaNumber(searchCategory.lawTotalCount)}건]</span></h1>
            </div>
            ...
            {searchCategory.lawTotalCount > 4 && (
            <div className="result-more">
                <Link href={`/unify/searchList?${queryString}&searchMenu=law`}>검색결과 더 보기</Link>
            </div>
            )}
            </div>
        </div>
