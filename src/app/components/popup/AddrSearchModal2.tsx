...
interface AddrSearchModalProps {
  open: boolean
  onCloseClick: () => void
  onSaveClick: (row: AddrRow, daddr: string) => void
}
...
export const AddrSearchModal = (props: AddrSearchModalProps) => {
  ...
  const { open, onCloseClick, onSaveClick } = props

  const [selectedRow, setSelectedRow] = useState<AddrRow>()
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      //`&confmKey=devU01TX0FVVEgyMDI1MDExMTEzMDEwMjExNTM5NjA=` 행정망
  ...
  const handleRowClick = (row: AddrRow) => {
    setSelectedRow(row)
  }
            ...
            <Box>
              <TableContainer>
                <Table className="table table-bordered">
                  <TableBody>
                    <TableRow>
                      <TableCell className="td-head td-left">주소</TableCell>
                      <TableCell className="td-left">
                        {selectedRow
                          ? `(${selectedRow?.zipNo}) ${selectedRow?.roadAddr}`
                          : ''}
                      </TableCell>
                    </TableRow>
                    <TableRow>
                      ...
                      <TableCell>
                        <div style={{ display: 'flex', gap: '8px' }}>
                          ...
                          <Button
                            variant="contained"
                            onClick={() =>
                              onSaveClick(selectedRow, String(params.daddr ?? ''))
                            }
                          >
                            저장
                          </Button>
