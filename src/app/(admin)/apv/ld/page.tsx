...
const DataList = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/apv/ld/cm/getAllLbrctDtls?page=${params.page}&size=${params.size}` +
  ...
  const fetchDetailData = async () => {
    ...
    try {
      let endpoint =
        `/fsm/apv/ld/cm/getAllDelngDtls?page=${detailParams.page}&size=${detailParams.size}` +
  ...
  const detailExcelDownload = async () => {
    let endpoint: string =
      `/fsm/apv/ld/cm/getExcelDelngDtls?` +
      `${detailParams.taskSeCd ? '&taskSeCd=' + detailParams.taskSeCd : ''}` +
      `${detailParams.locgovCd ? '&locgovCd=' + detailParams.locgovCd : ''}` +
      `${detailParams.dlngBgngYmd ? '&dlngBgngYmd=' + detailParams.dlngBgngYmd : ''}` +
      `${detailParams.dlngEndYmd ? '&dlngEndYmd=' + detailParams.dlngEndYmd : ''}` +
  ...
  return (
    <PageContainer title="주유내역" description="주유내역">
      ...
      <Box>
        <TableDataGrid
          headCells={vhclFalg ? apvLdVhclHC : apvLdHC} // 테이블 헤더 값
          rows={rows} // 목록 데이터
