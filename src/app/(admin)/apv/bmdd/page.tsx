...
import { apvlBmddDtlHC, apvlBmddHC } from '@/utils/fsms/headCells'
...
const DataList = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/apv/bmdd/bs/getAllBusMnbyDelngDtls?page=${params.page}&size=${params.size}` +
  ...
  const fetchDetailData = async () => {
    ...
    try {
      let endpoint =
        `/fsm/apv/bmdd/bs/getAllBusDelngDtls?page=${detailParams.page}&size=${detailParams.size}` +
  ...
  const excelDownload = async () => {
    ...
    try {
      let endpoint: string =
        `/fsm/apv/bmdd/bs/getExcelBusMnbyDelngDtls?page=${params.page}&size=${params.size}` +
        `${params.bgngDt ? '&bgngDt=' + params.bgngDt.replaceAll('-', '') : ''}` +
        `${params.endDt ? '&endDt=' + params.endDt.replaceAll('-', '') : ''}` +
        `${!isLocgovCdAll && params.ctpvCd ? '&ctpvCd=' + params.ctpvCd : ''}` +
        `${!isLocgovCdAll && params.locgovCd ? '&locgovCd=' + params.locgovCd : ''}` +
      ...
      link.href = url
      link.setAttribute('download', '버스월별거래내역.xlsx')
  ...
  const excelDetailDownload = async () => {
    ...
    try {
      let endpoint =
      `/fsm/apv/bmdd/bs/getExcelBusDelngDtls?page=${detailParams.page}&size=${detailParams.size}` +
      `${detailParams.dlngYm ? '&dlngYm=' + detailParams.dlngYm : ''}` +
      `${detailParams.locgovCd ? '&locgovCd=' + detailParams.locgovCd : ''}` +
      `${detailParams.brno ? '&brno=' + detailParams.brno : ''}` +
      `${detailParams.vhclSeCd ? '&vhclSeCd=' + detailParams.vhclSeCd : ''}` +
      ...
      const link = document.createElement('a')
      link.href = url
      link.setAttribute('download', '버스월별상세거래내역.xlsx')
      document.body.appendChild(link)
      link.click()
      // if (response && response.resultType === 'success' && response.data) {
      //   // 데이터 조회 성공시
  ...
  const handleModalOpen = async () => {
    ...
    try {
      ...
      let endPoint =
        `/fsm/apv/bmdd/bs/getAllBusStlmDtls?` +
  ...
  return (
    <PageContainer title="버스월별거래내역" description="버스월별거래내역">
      {/* breadcrumb */}
      <Breadcrumb title="버스월별거래내역" items={BCrumb} />
      {/* end breadcrumb */}
      ...
      <Box>
        ...
        <VhclHisSearchModal
          ...
          title="차량이력 조회"
          url="/fsm/apv/bmdd/bs/getAllVhcleMngHis"
          open={openH}
