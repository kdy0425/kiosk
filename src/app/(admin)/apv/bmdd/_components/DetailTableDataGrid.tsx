...
const DetailTableDataGrid: React.FC<ServerPaginationGridProps> = ({
  ...
  const [selectedRow, setSelectedRow] = useState<DetailRow>();
  ...
  // 행 클릭시 해당 데이터 set
  const onRowClick = (row: DetailRow, index: number) => {
    setSelectedRow(row);
  ...
  const getCardHistory = async () => {
    try {
      let endpoint = `/fsm/apv/bmdd/bs/getAllBusStlmDtls?` + 
      ...
      if(response && response.resultType === 'success' && response.data) {
        ...
        res.dlngDate = getDateTimeString(date)?.dateString;
        res.dlngTime = getDateTimeString(date)?.timeString;
  ...
  const getVehicleHistory = async () => {
    try {
      let endpoint = `/fsm/apv/bmdd/bs/getAllVhcleMngHis?` + 
