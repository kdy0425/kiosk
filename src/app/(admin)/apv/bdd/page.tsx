...
import { apvlBddHC } from '@/utils/fsms/headCells'
...
const DataList = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/apv/bdd/bs/getAllBusDelngDtls?page=${params.page}&size=${params.size}` +
  ...
  const excelDownload = async () => {
    ...
    try {
      let endpoint: string =
        `/fsm/apv/bdd/bs/getExcelBusDelngDtls?page=${params.page}&size=${params.size}` +
        `${params.sort ? '&sort=' + sortChange(params.sort) : ''}` +
        `${!isLocgovCdAll && params.ctpvCd ? '&ctpvCd=' + params.ctpvCd : ''}` +
        `${!isLocgovCdAll && params.locgovCd ? '&locgovCd=' + params.locgovCd : ''}` +
        `${params.bgngDt ? '&bgngDt=' + params.bgngDt.replaceAll('-', '') : ''}` +
  ...
  const handleModalOpen = async () => {
    ...
    try {
      ...
      let endPoint =
        `/fsm/apv/bdd/bs/getAllBusStlmDtls?` +
  ...
  return (
    <PageContainer title="버스거래내역" description="버스거래내역">
      {/* breadcrumb */}
        ...
        <Box className="table-bottom-button-group">
          <div className="button-right-align">
            ...
            <CardApvModal
              ...
              stlmAprvYmd={selectedRow ? (selectedRow.stlmAprvYmd ?? '') : ''}
              stlmAprvTm={selectedRow ? (selectedRow.stlmAprvTm ?? '') : ''}
              stlmCardNo={selectedRow ? (selectedRow.stlmCardNo ?? '') : ''}
            />
            <VhclHisSearchModal
              onCloseClick={() => setOpenH(false)}
              title="차량이력 조회"
              url="/fsm/apv/bdd/bs/getAllVhcleMngHis"
              open={openH}
              row={selectedRow}
            />
          </div>
