...
import { apvCoamTrHeadCells } from '@/utils/fsms/headCells'
...
const DataList = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/apv/coam/tr/getAllCousmOlt?page=${params.page}&size=${params.size}` +
        `${params.frcsBrno ? '&frcsBrno=' + params.frcsBrno.replaceAll('-', '') : ''}` +
        `${params.frcsNm ? '&frcsNm=' + params.frcsNm : ''}` +
        `${params.searchStDate ? '&bltBgngDt=' + params.searchStDate.replaceAll('-', '') : ''}` +
        `${params.searchEdDate ? '&bltEndDt=' + params.searchEdDate.replaceAll('-', '') : ''}`
  ...
  const excelDownload = async () => {
    ...
    try{
      ...
      let endpoint: string =
      `/fsm/apv/coam/tr/getExcelCousmOlt?` +
      `${params.frcsBrno ? '&frcsBrno=' + params.frcsBrno.replaceAll('-', '') : ''}` +
      `${params.frcsNm ? '&frcsNm=' + params.frcsNm : ''}` +
      `${params.searchStDate ? '&bltBgngDt=' + params.searchStDate.replaceAll('-', '') : ''}` +
      `${params.searchEdDate ? '&bltEndDt=' + params.searchEdDate.replaceAll('-', '') : ''}`
  ...
  const openUpdateModal = () => {
    if(selectedIndex > -1 && selectedRow) {
      if(selectedRow.bltSttsCd === '0'){
        setUpdateRemoteFlag(true);  
      }else{
        alert('지정 상태일 때만 수정 가능합니다.')
      }
    }else{
      alert('선택된 데이터가 없습니다')
    }
  ...
  return (
    ...
    >
      ...
      <Box>
        <TableDataGrid
          headCells={apvCoamTrHeadCells} // 테이블 헤더 값
          rows={rows} // 목록 데이터
