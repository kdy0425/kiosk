...
const DataList = () => {
    ...
    try {
      ...
      if (params.taskSeCd === 'TX') {
        if (!params.vhclNo) {
          ...
          if (!diffDate(params.searchStDate, params.searchEdDate, 6)) {
      ...
      let endpoint: string =
        `/fsm/apv/mdd/cm/getAllMnbyDelngDtls?page=${params.page}&size=${params.size}` +
  ...
  const excelDownload = async () => {
    ...
    let endpoint: string =
      `/fsm/apv/mdd/cm/getExcelMnbyDelngDtls?` +
      `${params.searchStDate ? '&dlngBgngYmd=' + params.searchStDate : ''}` +
      `${params.searchEdDate ? '&dlngEndYmd=' + params.searchEdDate : ''}` +
      `${params.ctpvCd ? '&ctpvCd=' + params.ctpvCd : ''}` +
      `${params.locgovCd ? '&locgovCd=' + params.locgovCd : ''}` +
  ...
  const fetchDetailData = async () => {
    ...
    try {
      let endpoint =
        `/fsm/apv/mdd/cm/getAllDelngDtls?page=${detailParams.page}&size=${detailParams.size}` +
  ...
  const detailExcelDownload = async () => {
    let endpoint: string =
      `/fsm/apv/mdd/cm/getExcelDelngDtls?page=${detailParams.page}&size=${detailParams.size}` +
      `${detailParams.taskSeCd ? '&taskSeCd=' + detailParams.taskSeCd : ''}` +
      `${detailParams.locgovCd ? '&locgovCd=' + detailParams.locgovCd : ''}` +
      `${detailParams.dlngYm ? '&dlngYm=' + detailParams.dlngYm : ''}` +
      `${detailParams.vhclNo ? '&vhclNo=' + detailParams.vhclNo : ''}` +
...
await  getExcelFile(
  ...
  return (
    <PageContainer title="월별거래내역" description="월별거래내역">
      {/* breadcrumb */}
      <Breadcrumb title="월별거래내역" items={BCrumb} />
      {/* end breadcrumb */}
      ...
      <Box>
        <TableDataGrid
          headCells={vhclFalg ? apvMddVhclHC : apvMddHC} // 테이블 헤더 값
          rows={rows} // 목록 데이터
