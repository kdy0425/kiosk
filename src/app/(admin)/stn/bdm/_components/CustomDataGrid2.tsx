...
const TableDataGrid: React.FC<ServerPaginationGridProps> = ({
  ...
  function isRowspan(index: number, id: string) {
    if (index > 0 && 
        rows[index]['groupId'] && (rows[index - 1]['groupId'] === rows[index]['groupId']) && 
        rows[index]['vhclNo'] && (rows[index - 1]['vhclNo'] === rows[index]['vhclNo']) && 
        rows[index]['chgYmd'] && (rows[index - 1]['chgYmd'] === rows[index]['chgYmd']) && 
        rows[index]['brno'] && (rows[index - 1]['brno'] === rows[index]['brno']) &&
        rows[index]['groupNm'] && (rows[index - 1]['groupNm'] === rows[index]['groupNm']) &&
        rows[index]['rprsvNm'] && (rows[index - 1]['rprsvNm'] === rows[index]['rprsvNm'])
    ) {
      return true
    } else {
      return false
  ...
  function getRowspan(index: number, id: string) {
    let count = 1
    const groupId = rows[index]['groupId']
    const vhclNo = rows[index]['vhclNo']
    const chgYmd = rows[index]['chgYmd']
    const brno = rows[index]['brno']
    const groupNm = rows[index]['groupNm']
    const rprsvNm = rows[index]['rprsvNm']

    if (!rows[index]) return count

    for (let i = index + 1; i < rows.length; i++) {
      if (rows[i]['groupId'] === groupId && 
          rows[i]['vhclNo'] === vhclNo && 
          rows[i]['chgYmd'] === chgYmd && 
          rows[i]['brno'] === brno && 
          rows[i]['groupNm'] === groupNm && 
          rows[i]['rprsvNm'] === rprsvNm
      ) {
        count++
      } else {
        break
                        ...
                        {headCells.map((headCell, i) => (
                          <React.Fragment key={'tr' + index + i + headCell.id}>
                            {headCell.format === 'button' ? (
                              ...
                              // Check if this row should display a checkbox based on rowspan logic
                              isRowspan(index, headCell.id) ? null : (
                                <TableCell
                                  rowSpan={
                                    headCell.rowspan && !isRowspan(index, headCell.id)
                                ...
                                >
                                  <CustomCheckbox
                                    ...
                                    onChange={() => handleSelect('tr' + index, row, index)} // row와 index 추가
                                  />
                                </TableCell>)
                            ) : (headCell.rowspan &&
