...
const BusPage = () => {
  ...
  useEffect(() => {
    if ('locgovCd' in authInfo && authInfo.locgovCd && selectedIndex > -1) {
      if (authInfo.locgovCd != rows[selectedIndex].exsLocgovCd) {
        setIsCurrSameLocgov(
          authInfo.locgovCd === rows[selectedIndex].chgLocgovCd &&
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/stn/lttm/bs/getAllLgovTrnsfrnRequst?page=${params.page}&size=${params.size}` +
  ...
  const putData = async (row: Row, sttCode: string) => {
    ...
    try {
      ...
      //승인
      if (sttCode === '02')
        endpoint = `/fsm/stn/lttm/bs/approveLgovTrnsfrnRequst`

      //거절
      if (sttCode === '03')
        endpoint = `/fsm/stn/lttm/bs/rejectLgovTrnsfrnRequst`

      //취소
      if (sttCode === '04')
        endpoint = `/fsm/stn/lttm/bs/cancelLgovTrnsfrnRequst`
  ...
  const excelDownload = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/stn/lttm/bs/getExcelLgovTrnsfrnRequst?` +
        `${params.ctpvCd ? '&ctpvCd=' + params.ctpvCd : ''}` +
        `${params.locgovCd ? '&locgovCd=' + params.locgovCd : ''}` +
        `${params.bgngDt ? (('&bgngDt=' + (params.bgngDt + '').replaceAll('-', '')) as string) : ''}` +
        `${params.endDt ? (('&endDt=' + (params.endDt + '').replaceAll('-', '')) as string) : ''}` +
  ...
  return (
    ...
    >      
      ...
      <LocHistoryModal
        ...
        data={selectedRow as {}}
        type={'bs'}
        url={'/fsm/stn/lttm/bs/getAllLgovMvtHst'}
      />
