...
const TaxiPage = () => {
  ...
  const radioList: radioListType[] = [{ label: '지자체이관 전입 단건등록', value: 'single' }, { label: '지자체이관 전입 일괄등록', value: 'multi' }]
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const endpoint = '/fsm/stn/lttm/tx/getAllLgovTrnsfrnRequst' + toQueryParameter(searchObj)
  ...
  const putData = async (actionType: string, procType: 'single' | 'multi', rfslRsnCn?: string) => {
    ...
    try {
      ...
      const endpoint = procType === 'single' ? '/fsm/stn/lttm/tx/updateLgovTrnsfrnRequst' : '/fsm/stn/lttm/tx/updateLgovTrnsfrnMultiRequst'
  ...
  const handleSingleProcess = (actionType: '01' | '02' | '04') => {
    ...
    if (!(selectedRow && selectedRow.prcsSttsCd == '00' && selectedRow.dmndSeCd == '2' && selectedRow.chgLocgovCd === authLocgovCd)) {
  ...
  const handleMultiProcess = (actionType: '01' | '02' | '04'): void => {
    ...
    try {
      ...
      for (let i = 0; i < checkedRows.length; i++) {
        ...
        if (!(rows[num].prcsSttsCd == '00' && rows[num].dmndSeCd == '2' && rows[num].chgLocgovCd === authLocgovCd)) {
          flag = false
          break;
        }
      }
  ...
  const excelDownload = async () => {
    ...
    try {
      setLoadingBackdrop(true)
      let endpoint: string =
        `/fsm/stn/lttm/tx/getExcelLgovTrnsfrnRequst?` +
  ...
  const handleSelectAllInterceptor = useCallback((pRows: Row[]): Row[] => {
    const resultRows: Row[] = pRows.filter(item => (item.prcsSttsCd === '00' && item.dmndSeCd === '2' && item.chgLocgovCd === authLocgovCd))
  ...
  const handleSelectInterceptor = useCallback((pRow: Row): boolean => {
    if (pRow.prcsSttsCd === '00' && pRow.dmndSeCd === '2' && pRow.chgLocgovCd === authLocgovCd) {
      return true
    } else {
      alert('처리상태가 [이관요청] 및 요청구분명이 [전출요청]인 데이터 중,\n전입관청이 본인 지자체로 등록된 데이터만 선택 가능합니다.')
      return false
    }
  }, [authLocgovCd])
          ...
          {selectedRow && (
            <BlankCard className="contents-card" title="상세 정보"
              buttons={[
                ...
                {
                  label: '승인',
                  disabled: !(selectedRow.prcsSttsCd == '00' && selectedRow.dmndSeCd == '2' && selectedRow.chgLocgovCd === authLocgovCd),
