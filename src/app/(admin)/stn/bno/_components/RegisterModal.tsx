...
import { stnbnoRegisterModalHeadCells } from '@/utils/fsms/headCells';
...
export const RegisterModal = (props: RegisterModal) => {
    ...
    const fetchData = async () => {
        ...
        let endpoint: string =
            `/fsm/stn/bno/cm/getByegNtfcOilprcTemplete`
    ...
    const handleAllRegister = () => {
        ...
        //모두 업데이터는 날짜와 가격만 일괄적으로 업데이트
        const updatedRows = rows.map((row) => ({
            ...row,
            aplcnBgngYmd: data.aplcnBgngYmd, // 날짜 업데이트
            ancmntOilprcAmt: data.ancmntOilprcAmt, // 가격 업데이트
        }));
        setRows(updatedRows);
        ...
        alert('모든 행이 일괄 등록되었습니다.');
    };

    const saveData = async () => {
        if (!confirm('지역별 고시유가를 수정하시겠습니까?')) return;
        let endpoint: string = `/fsm/stn/bno/cm/createByegNtfcOilprc`;
...
return (
        ...
        <DialogContent>
            ...
            <TableDataGrid
                headCells={stnbnoRegisterModalHeadCells} // 테이블 헤더 값
                rows={rows} // 목록 데이터
