...
const FormDialog: React.FC<FormDialogProps> = ({ size, detail, open, handleClose }) => {
  ...
  const fetchHistoryData = async (row: Row) => {
    try {
      let endpoint: string = 
        `/fsm/ilg/ssp/tr/getAllSbsidyStopPymntHst?` + 
      ...
      if (response && response.resultType === 'success' && response.data) {
        setHistoryRow(response.data.content)
      } else {
        setHistoryRow([]);
      }
    } catch (error) {
  ...
  <>
    <Dialog fullWidth={false} maxWidth={size} open={open} onClose={handleClose}>
        <DialogContent>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', m: 'auto', width: 'full' }}>
                <TableDataGrid 
