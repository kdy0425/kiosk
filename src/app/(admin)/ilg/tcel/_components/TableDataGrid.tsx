...
import { openStdTnkCpctyModal } from '@/store/popup/StdTnkCpctySlice'
...
const TableDataGrid: React.FC<ServerPaginationGridProps> = ({
  ...
  const tcelInfo = useSelector((state: AppState) => state.tcelInfo)
              ...
              <React.Fragment key={'th' + index}>
                {headCell.format === 'checkbox' ? (
                  <TableCell padding="checkbox">
                    <CustomCheckbox
                      indeterminate={
                        tcelSelectedData.length > 0 &&
                        tcelSelectedData.length < rows.length
                      }
  ...
  const { tcelSelectedData, tcelExamResultData } = tcelInfo
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center' }}>
            {tabIndex === '1' ? (
              <DoubtDelngButtonGroup
                ...
                handleBypeExamTrgetEach={() =>
                  handleBypeExamTrgetEach(tcelSelectedData)
                }
              />
            ) : null}
            {tabIndex === '2' ? (
              <ExamTrgetButtonGroup
                handleCreateExamResult={() =>
                  handleCreateExamResult(tcelSelectedData)
                }
                handleCreateLgovTr={() => handleCreateLgovTr(tcelSelectedData)}
                handleCancelExamTrget={() =>
                  handleCancelExamTrget(tcelSelectedData)
                }
              />
            ) : null}
            {tabIndex === '3' ? (
              <ExamResultButtonGroup
                handleCreateExaathr={() =>
                  handleCreateExaathr(tcelExamResultData)
                }
                handleCancelExamResult={() =>
                  handleCancelExamResult(tcelExamResultData)
                }
            ...
            {tabIndex === '4' ? (
              <ExaathrButtonGroup
                handleCancelExaathr={() =>
                  handleCancelExaathr(tcelSelectedData)
                }
                handleChange={handleRadioChange}
              />
            ) : null}
            {tabIndex === '5' ? (
              <LgovTrfCfmButtonGroup
                handleLgovTrfConfirm={() =>
                  handleUpdateLgovTrf(tcelSelectedData, 'A')
                }
                handleLgovTrfDeny={() =>
                  handleUpdateLgovTrf(tcelSelectedData, 'D')
                }
                handleChange={handleRadioChange}
              />
            ) : null}
            {tabIndex === '6' ? (
              <LgovTrfntfReqButtonGroup
                handleLgovTrfCancel={() =>
                  handleUpdateLgovTrf(tcelSelectedData, 'C')
                }
                handleChange={handleRadioChange}
              />
            ) : null}
        ...
        <TableContainer>
          <Table aria-labelledby="tableTitle" size={'medium'}>
            ...
            <EnhancedTableHead
              ...
              numSelected={tcelSelectedData.length}
            />
            {/* 테이블의 내용부분  */}
            <TableBody>
              {!loading ? (
