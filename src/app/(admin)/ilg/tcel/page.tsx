...
import StdTnkModal from '@/app/components/tr/popup/StdTnkModal'
...
const DataList = () => {
  ...
  const stdTnkCpctyInfo = useSelector((state: AppState) => state.StdTnkInfo)

  // 플래그를 통한 데이터 갱신
  // 플래그의 변화를 통해 현재 정보를 기준으로 데이터를 가져오기위해 설정
  useEffect(() => {
    if (tcelInfo.tcelSearchDone || flag) {
      fetchData()
    }
  }, [tcelInfo.tcelSearchDone, flag])
  ...
  const returnHeadCell = (tabValue: string): HeadCell[] => {
    return tcelTabHeadCells[Number(tabValue) - 1]
  }
  ...
  return (
    <PageContainer title="탱크용량초과주유" description="탱크용량초과주유">
      {/* breadcrumb */}
      <Breadcrumb title="탱크용량초과주유" items={BCrumb} />
      {/* end breadcrumb */}
      ...
      <>
        {decisionBypeInfo.dbModalOpen ? <DecisionBypeModal dwNo="04" /> : null}
        ...
        {stdTnkCpctyInfo.STCModalOpen ? <StdTnkModal /> : null}
      </>
    </PageContainer>
  )
}
