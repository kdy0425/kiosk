...
// 테이블 caption
const tableCaption: string = '톤급별평균대비초과주유 의심거래내역'
...
const TableDataGrid: React.FC<ServerPaginationGridProps> = ({
  ...
  const taavelInfo = useSelector((state: AppState) => state.taavelInfo)
  ...
  function EnhancedTableHead(props: Readonly<EnhancedTableProps>) {
    ...
    const { taavelSelectedData } = taavelInfo
    return (
      <TableHead>
        <TableRow key={'thRow'}>
                ...
                {headCell.format === 'checkbox' ? (
                  <TableCell padding="checkbox">
                    ...
                    <CustomCheckbox
                      id="allChk"
                      indeterminate={
                        taavelSelectedData.length > 0 &&
                        taavelSelectedData.length < rows.length
                      }
  ...
  const { taavelSelectedData, taavelExamResultData } = taavelInfo
  return (
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center' }}>
            {tabIndex === '1' ? (
              <DoubtDelngButtonGroup
                ...
                handleBypeExamTrgetEach={() =>
                  handleBypeExamTrgetEach(taavelSelectedData)
                }
              />
            ) : null}
            {tabIndex === '2' ? (
              <ExamTrgetButtonGroup
                handleCreateExamResult={() =>
                  handleCreateExamResult(taavelSelectedData)
                }
                handleCreateLgovTr={() =>
                  handleCreateLgovTr(taavelSelectedData)
                }
                handleCancelExamTrget={() =>
                  handleCancelExamTrget(taavelSelectedData)
                }
              />
            ) : null}
            {tabIndex === '3' ? (
              <ExamResultButtonGroup
                handleCreateExaathr={() =>
                  handleCreateExaathr(taavelExamResultData)
                }
                handleCancelExamResult={() =>
                  handleCancelExamResult(taavelExamResultData)
                }
            ...
            {tabIndex === '4' ? (
              <ExaathrButtonGroup
                handleCancelExaathr={() =>
                  handleCancelExaathr(taavelSelectedData)
                }
                handleChange={handleRadioChange}
              />
            ) : null}
            {tabIndex === '5' ? (
              <LgovTrfCfmButtonGroup
                handleLgovTrfConfirm={() =>
                  handleUpdateLgovTrf(taavelSelectedData, 'A')
                }
                handleLgovTrfDeny={() =>
                  handleUpdateLgovTrf(taavelSelectedData, 'D')
                }
                handleChange={handleRadioChange}
              />
            ) : null}
            {tabIndex === '6' ? (
              <LgovTrfntfReqButtonGroup
                handleLgovTrfCancel={() =>
                  handleUpdateLgovTrf(taavelSelectedData, 'C')
                }
                handleChange={handleRadioChange}
              />
            ) : null}
        ...
        <TableContainer>
          <Table aria-labelledby="tableTitle" size={'medium'}>
            ...
            <EnhancedTableHead
              ...
              numSelected={taavelSelectedData.length}
            />
            {/* 테이블의 내용부분  */}
            <TableBody>
              {!loading ? (
