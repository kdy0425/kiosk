...
import HeaderTab from '@/app/components/tables/HeaderTab'
...
import { getToday, getExcelFile } from '@/utils/fsms/common/comm'
...
const DataList = () => {
  ...
  useEffect(() => {
    if (taavelInfo.taavelSearchDone || flag) {
      fetchData()
    }
  }, [taavelInfo.taavelSearchDone, flag])
  ...
  const returnHeadCell = (tabValue: string): HeadCell[] => {
    return taavelTabHeadCells[Number(tabValue) - 1]
  }
  ...
  return (
    <PageContainer
      title="톤급별평균대비초과주유"
      description="톤급별평균대비초과주유"
    >
      {/* breadcrumb */}
      <Breadcrumb title="톤급별평균대비초과주유" items={BCrumb} />
      {/* end breadcrumb */}
