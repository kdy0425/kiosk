...
// headCells
import { ilgBzeeDoubtTransTxHC } from '@/utils/fsms/headCells' // 의심거래내역 탭 그리드
import { ilgBzeeExamTargetTxHC } from '@/utils/fsms/headCells' // 조사대상내역 탭 그리드
...
import OtherLocalInfoModal from './_components/OtherLocalInfoModal'
...
const DataList = () => {
      ...
      tabRows.forEach((tabRow, tabIdx) => {
        if (sumRow.exmnNo === tabRow.exmnNo) {
          if (!sumFlag) {
            ...
            sumRows[sumIdx].opsAmt = String(
              Number(sumRow.opsAmt === '' ? '0' : sumRow.opsAmt) +
                Number(tabRow.opsAmt === '' ? '0' : tabRow.opsAmt)
            )
          }
        }
      }),
  ...
  return (
    <PageContainer
      title="사업구역 외 충전"
      description="사업구역 외 충전"
    >
      {/* breadcrumb */}
      <Breadcrumb title="사업구역 외 충전" items={BCrumb} />
      {/* end breadcrumb */}
      ...
      <div style={{ marginTop: '-8px' }}>
        <BlankCard>
          <Box display={selectedTab === '0' ? 'block' : 'none'} sx={{ mb: 2 }}>
            ...
            <TabDataGrid
              headCells={ilgBzeeDoubtTransTxHC} // 테이블 헤더 값
              rows={tabRows} // 목록 데이터
          ...
          <Box display={selectedTab === '1' ? 'block' : 'none'} sx={{ mb: 2 }}>
            ...
            <TabDataGrid
              headCells={ilgBzeeExamTargetTxHC} // 테이블 헤더 값
              rows={tabRows} // 목록 데이터
              totalRows={tabTotalRows} // 총 로우 수
              loading={loading} // 로딩여부
              onPaginationModelChange={handlePaginationModelChange} // 페이지 , 사이즈 변경 핸들러 추가
