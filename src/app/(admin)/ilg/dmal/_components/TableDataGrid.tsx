...
const TableDataGrid: React.FC<ServerPaginationGridProps> = ({
  ...
  const dmalInfo = useSelector((state: AppState) => state.dmalInfo)
  ...
  function EnhancedTableHead(props: Readonly<EnhancedTableProps>) {
    ...
    const { dmalSelectedData } = dmalInfo
    return (
      <TableHead>
        <TableRow key={'thRow'}>
                ...
                {headCell.format === 'checkbox' ? (
                  <TableCell padding="checkbox">
                    ...
                    <CustomCheckbox
                      id="chkAll"
                      indeterminate={
                        dmalSelectedData.length > 0 &&
                        dmalSelectedData.length < rows.length
                      }
  ...
  const { dmalSelectedData, dmalExamResultData } = dmalInfo
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center' }}>
            {tabIndex === '1' ? (
              <DoubtDelngButtonGroup
                ...
                handleBypeExamTrgetEach={() =>
                  handleBypeExamTrgetEach(dmalSelectedData)
                }
              />
            ) : null}
            {tabIndex === '2' ? (
              <ExamTrgetButtonGroup
                handleCreateExamResult={() =>
                  handleCreateExamResult(dmalSelectedData)
                }
                handleCreateLgovTr={() => handleCreateLgovTr(dmalSelectedData)}
                handleCancelExamTrget={() =>
                  handleCancelExamTrget(dmalSelectedData)
                }
              />
            ) : null}
            {tabIndex === '3' ? (
              <ExamResultButtonGroup
                handleCreateExaathr={() =>
                  handleCreateExaathr(dmalExamResultData)
                }
                handleCancelExamResult={() =>
                  handleCancelExamResult(dmalExamResultData)
                }
            ...
            {tabIndex === '4' ? (
              <ExaathrButtonGroup
                handleCancelExaathr={() =>
                  handleCancelExaathr(dmalSelectedData)
                }
                handleChange={handleRadioChange}
              />
            ) : null}
            {tabIndex === '5' ? (
              <LgovTrfCfmButtonGroup
                handleLgovTrfConfirm={() =>
                  handleUpdateLgovTrf(dmalSelectedData, 'A')
                }
                handleLgovTrfDeny={() =>
                  handleUpdateLgovTrf(dmalSelectedData, 'D')
                }
                handleChange={handleRadioChange}
              />
            ) : null}
            {tabIndex === '6' ? (
              <LgovTrfntfReqButtonGroup
                handleLgovTrfCancel={() =>
                  handleUpdateLgovTrf(dmalSelectedData, 'C')
                }
                handleChange={handleRadioChange}
              />
            ) : null}
        ...
        <TableContainer>
          <Table aria-labelledby="tableTitle" size={'medium'}>
            ...
            <EnhancedTableHead
              ...
              numSelected={dmalSelectedData.length}
            />
            {/* 테이블의 내용부분  */}
            <TableBody>
              {!loading ? (
