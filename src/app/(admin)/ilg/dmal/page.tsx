...
const DataList = () => {
  ...
  useEffect(() => {
    if (dmalInfo.dmalSearchDone || flag) {
      fetchData()
    }
  }, [dmalInfo.dmalSearchDone, flag])
  ...
  const returnHeadCell = (tabValue: string): HeadCell[] => {
    return dmalTabHeadCells[Number(tabValue) - 1]
  }
  ...
  return (
    <PageContainer title="1일4회이상주유" description="1일4회이상주유">
      {/* breadcrumb */}
      <Breadcrumb title="1일4회이상주유" items={BCrumb} />
      {/* end breadcrumb */}
      ...
      <>
        {decisionBypeInfo.dbModalOpen ? <DecisionBypeModal dwNo="03" /> : null}
