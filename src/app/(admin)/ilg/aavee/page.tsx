...
import DtlDataGrid from '@/app/components/tables/CommDataGrid2'
import AvgDataGrid from './_components/AverageTableDataGrid'
...
// headCells
import { ilgAaveeAvgTxHC } from '@/utils/fsms/headCells' // 지역평균 누적거래내역 집계 그리드 (상단)
import { ilgAaveeDtlTxHC } from '@/utils/fsms/headCells' // 차량별 거래내역 상세 그리드 (하단)
import { ilgAaveeDoubtTransTxHC } from '@/utils/fsms/headCells' // 의심거래내역 탭 그리드
import { ilgAaveeExamTargetTxHC } from '@/utils/fsms/headCells' // 조사대상내역 탭 그리드
...
const DataList = () => {
  ...
  const [tabLoading, setTabLoading] = useState(false) // 탭 그리드 로딩 상태
  const [avgLoading, setAvgLoading] = useState(false) // 지역평균 누적거래 집계 로딩 상태
  const [dtlLoading, setDtlLoading] = useState(false) // 차량별 거래내역 상세 로딩 상태
  const [loadingBackdrop, setLoadingBackdrop] = useState(false) // 저장/수정/삭제 시 로딩 상태
  ...
  const [avgFlag, setAvgFlag] = useState<boolean>(false)
  const [avgRows, setAvgRows] = useState<Row[]>([])
  const [avgTotalRows, setAvgTotalRows] = useState(0) // 총 수

  const [dtlShowFlag, setDtlShowFlag] = useState<boolean>(false)
  ...
  const [avgPageable, setAvgPageable] = useState<pageable>({
  ...
  const [dtlPageable, setDtlPageable] = useState<Pageable2>({
    pageNumber: 1, // 페이지 번호는 1부터 시작
    pageSize: 10, // 기본 페이지 사이즈 설정
    totalPages: 1, // 정렬 기준
  })
  ...
  const fetchTab = async () => {
    ...
    try {
      ...
      if (response && response.resultType === 'success' && response.data) {
        ...
        if (response.data.totalElements > 0 && selectedTab === '0') {
  ...
  const fetchAvg = async () => {
    try {
      ...
      const endpoint: string =
        `/fsm${BCrumb[BCrumb.length - 1].to}/tx/getAllDoubtAreaAvgStatList?` +
        `${params.ctpvCd ? '&ctpvCd=' + params.ctpvCd : ''}` +
        `${params.locgovCd ? '&locgovCd=' + params.locgovCd : ''}` +
        `${params.bzmnSeCd ? '&bzmnSeCd=' + params.bzmnSeCd : ''}` +
        `${params.bgngDt ? '&bgngDt=' + params.bgngDt.replaceAll('-', '') : ''}` +
  ...
  // 지역평균 누적거래내역 집계 엑셀 다운로드
  const avgExcelDownload = async () => {
    const endpoint: string =
      `/fsm${BCrumb[BCrumb.length - 1].to}/tx/getExcelAllDoubtAreaAvgStatList?` +
      `${params.ctpvCd ? '&ctpvCd=' + params.ctpvCd : ''}` +
      `${params.locgovCd ? '&locgovCd=' + params.locgovCd : ''}` +
      `${params.bzmnSeCd ? '&bzmnSeCd=' + params.bzmnSeCd : ''}` +
      `${params.bgngDt ? '&bgngDt=' + params.bgngDt.replaceAll('-', '') : ''}` +
...
await  getExcelFile(
  ...
  // 차량별 거래내역 상세 조회
  const fetchDtl = async () => {
    try {
      setDtlLoading(true)

      // 검색 조건에 맞는 endpoint 생성
      const endpoint: string =
        `/fsm${BCrumb[BCrumb.length - 1].to}/tx/getAllDoubtAreaAprvDtlList?page=${dtlParams.page}&size=${dtlParams.size}` +
        `${'&dlngYm=' + dtlParams.dlngYm}` +
        `${'&ctpvCd=' + dtlParams.ctpvCd}` +
        `${'&locgovCd=' + dtlParams.locgovCd}` +
        `${'&brno=' + dtlParams.brno}` +
        `${'&vhclNo=' + dtlParams.vhclNo}`
      ...
      if (response && response.resultType === 'success' && response.data) {
        // 데이터 조회 성공시
        setDtlRows(response.data.content)
        setDtlTotalRows(response.data.totalElements)
        setDtlPageable({
          pageNumber: response.data.pageable.pageNumber + 1,
          pageSize: response.data.pageable.pageSize,
          totalPages: response.data.totalPages,
  ...
  // 차량별 거래내역 상세 엑셀 다운로드
  const dtlExcelDownload = async () => {
    const endpoint: string =
      `/fsm${BCrumb[BCrumb.length - 1].to}/tx/getExcelAllDoubtAreaAprvDtlList?` +
      `${'&dlngYm=' + dtlParams.dlngYm}` +
      `${'&ctpvCd=' + dtlParams.ctpvCd}` +
      `${'&locgovCd=' + dtlParams.locgovCd}` +
      `${'&brno=' + dtlParams.brno}` +
...
await  getExcelFile(
  ...
  // 세부내역 행 클릭 시 호출되는 함수
  function dtlRowClick() {}

  // 페이지 번호와 페이지 사이즈를 params에 업데이트
  const dtlPaginationModelChange = (page: number, pageSize: number) => {
    setDtlParams((prev) => ({
      ...prev,
      page: page,
      size: pageSize,
  ...
  function sumAmtPerExmnNo() {
      ...
      tabRows.forEach((tabRow, tabIdx) => {
        if (sumRow.exmnNo === tabRow.exmnNo) {
          if (!sumFlag) {
            ...
            sumRows[sumIdx].acmlAprvAmt = String(
              Number(sumRow.acmlAprvAmt === '' ? '0' : sumRow.acmlAprvAmt) +
                Number(tabRow.acmlAprvAmt === '' ? '0' : tabRow.acmlAprvAmt)
            )
    ...
    sumRows.map(
      (row) => (
        (row.chk = '0'),
        (row.totlAprvAmt = row.acmlAprvAmt),
  ...
  return (
    ...
    >
      ...
      {/* 테이블영역 시작 */}
      <Box
        className="table-top-button-group"
        style={{
      ...
      >
        ...
        >
          <AvgDataGrid
            ...
            headCells={ilgAaveeAvgTxHC}
            rows={avgRows}
            totalRows={avgTotalRows}
            loading={avgLoading}
            onPaginationModelChange={() => {}}
            onSortModelChange={handleSortModelChange}
            onRowClick={() => {}}
            pageable={avgPageable}
          />
        </BlankCard>
      </Box>
      <TxSearchHeaderTab
      ...
      <div style={{ marginTop: '-8px' }}>
        <BlankCard>
          <Box display={selectedTab === '0' ? 'block' : 'none'} sx={{ mb: 2 }}>
            ...
            <TabDataGrid
              headCells={ilgAaveeDoubtTransTxHC} // 테이블 헤더 값
              rows={tabRows} // 목록 데이터
              totalRows={tabTotalRows} // 총 로우 수
              loading={tabLoading} // 로딩여부
          ...
          <Box display={selectedTab === '1' ? 'block' : 'none'} sx={{ mb: 2 }}>
            ...
            <TabDataGrid
              headCells={ilgAaveeExamTargetTxHC} // 테이블 헤더 값
              rows={tabRows} // 목록 데이터
              totalRows={tabTotalRows} // 총 로우 수
              loading={tabLoading} // 로딩여부
              onRowClick={handleRowClick} // 행 클릭 핸들러 추가
      ...
      <div>
        {dtlShowFlag && (
          ...
          >
            <DtlDataGrid
              headCells={ilgAaveeDtlTxHC} // 테이블 헤더 값
              rows={dtlRows} // 목록 데이터
              totalRows={dtlTotalRows} // 총 로우 수
              loading={dtlLoading} // 로딩여부
              onRowClick={dtlRowClick} // 행 클릭 핸들러 추가
              onPaginationModelChange={dtlPaginationModelChange} // 페이지 , 사이즈 변경 핸들러 추가
              pageable={dtlPageable} // 현재 페이지 / 사이즈 정보
              paging={true}
            />
          </BlankCard>
        )}
