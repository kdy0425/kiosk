...
// 테이블 caption
const tableCaption: string = '거리대비주유시간 이상주유'
...
const TableDataGrid: React.FC<ServerPaginationGridProps> = ({
  ...
  const dvhalInfo = useSelector((state: AppState) => state.dvhalInfo)
  ...
  function EnhancedTableHead(props: Readonly<EnhancedTableProps>) {
    ...
    const { dvhalSelectedData } = dvhalInfo
    const headCellPart1 = headCells.slice(0, headCells.length - 7) //(0, 22)
    const headCellPart2 = headCells.slice(
      headCells.length - 7,
      headCells.length - 2,
    ) //(22, 27)
    const headCellPart3 = headCells.slice(
      headCells.length - 2,
      headCells.length,
    ) //(27, 29)
    return (
      <TableHead>
        {selectedTabIdx === '1' || selectedTabIdx === '2' ? (
          <>
            <TableRow key={'thRow1'}>
              {headCellPart1.map((headCell) => {
                return (
                  <React.Fragment key={'th' + headCell.id}>
                    {headCell.format === 'checkbox' ? (
                      <TableCell
                        rowSpan={headCell.isCol ? 2 : 1}
                        padding="checkbox"
                      >
                        <CustomFormLabel
                        ...
                        <CustomCheckbox
                          id="allChk"
                          indeterminate={
                            dvhalSelectedData.length > 0 &&
                            dvhalSelectedData.length < rows.length
                          }
              ...
              <TableCell
                ...
                colSpan={3}
              >
                <div className="table-head-text">TMAP제공 정보</div>
              </TableCell>
              <TableCell
                style={{ whiteSpace: 'nowrap' }}
                align={'left'}
                colSpan={2}
              >
                <div className="table-head-text">거래내역 정보</div>
              </TableCell>
              {headCellPart3.map((headCell) => {
                return (
                  <React.Fragment key={'th' + headCell.id}>
                    {headCell.format === 'checkbox' ? (
                      <TableCell
  ...
  const { dvhalSelectedData, dvhalExamResultData } = dvhalInfo
  return (
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center' }}>
            {tabIndex === '1' ? (
              <DoubtDelngButtonGroup
                ...
                handleBypeExamTrgetEach={() =>
                  handleBypeExamTrgetEach(dvhalSelectedData)
                }
              />
            ) : null}
            {tabIndex === '2' ? (
              <ExamTrgetButtonGroup
                handleCreateExamResult={() =>
                  handleCreateExamResult(dvhalSelectedData)
                }
                handleCreateLgovTr={() => handleCreateLgovTr(dvhalSelectedData)}
                handleCancelExamTrget={() =>
                  handleCancelExamTrget(dvhalSelectedData)
                }
              />
            ) : null}
            {tabIndex === '3' ? (
              <ExamResultButtonGroup
                handleCreateExaathr={() =>
                  handleCreateExaathr(dvhalExamResultData)
                }
                handleCancelExamResult={() =>
                  handleCancelExamResult(dvhalExamResultData)
                }
            ...
            {tabIndex === '4' ? (
              <ExaathrButtonGroup
                handleCancelExaathr={() =>
                  handleCancelExaathr(dvhalSelectedData)
                }
                handleChange={handleRadioChange}
              />
            ) : null}
            {tabIndex === '5' ? (
              <LgovTrfCfmButtonGroup
                handleLgovTrfConfirm={() =>
                  handleUpdateLgovTrf(dvhalSelectedData, 'A')
                }
                handleLgovTrfDeny={() =>
                  handleUpdateLgovTrf(dvhalSelectedData, 'D')
                }
                handleChange={handleRadioChange}
              />
            ) : null}
            {tabIndex === '6' ? (
              <LgovTrfntfReqButtonGroup
                handleLgovTrfCancel={() =>
                  handleUpdateLgovTrf(dvhalSelectedData, 'C')
                }
                handleChange={handleRadioChange}
              />
            ) : null}
        ...
        <TableContainer>
          <Table aria-labelledby="tableTitle" size={'medium'}>
            ...
            <EnhancedTableHead
              ...
              numSelected={dvhalSelectedData.length}
