...
// 테이블 caption
const tableCaption: string = '단시간반복주유 의심거래내역'
...
const TableDataGrid: React.FC<ServerPaginationGridProps> = ({
  ...
  // const lpavInfo = useSelector((state: AppState) => state.lpavInfo)
  const shlInfo = useSelector((state: AppState) => state.shlInfo)
                ...
                {headCell.format === 'checkbox' ? (
                  <TableCell padding="checkbox">
                    ...
                    <CustomCheckbox
                      id="allChk"
                      indeterminate={
                        shlSelectedData.length > 0 &&
                        shlSelectedData.length < rows.length
                      }
  ...
  const { shlSelectedData, shlExamResultData } = shlInfo
      ...
      <div className="data-grid-wrapper">
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center' }}>
            {tabIndex === '1' ? (
              <DoubtDelngButtonGroup
                ...
                handleBypeExamTrgetEach={() =>
                  handleBypeExamTrgetEach(shlSelectedData)
                }
              />
            ) : null}
            {tabIndex === '2' ? (
              <ExamTrgetButtonGroup
                handleCreateExamResult={() =>
                  handleCreateExamResult(shlSelectedData)
                }
                handleCreateLgovTr={() => handleCreateLgovTr(shlSelectedData)}
                handleCancelExamTrget={() =>
                  handleCancelExamTrget(shlSelectedData)
                }
              />
            ) : null}
            {tabIndex === '3' ? (
              <ExamResultButtonGroup
                handleCreateExaathr={() =>
                  handleCreateExaathr(shlExamResultData)
                }
                handleCancelExamResult={() =>
                  handleCancelExamResult(shlExamResultData)
                }
            ...
            {tabIndex === '4' ? (
              <ExaathrButtonGroup
                handleCancelExaathr={() => handleCancelExaathr(shlSelectedData)}
                handleChange={handleRadioChange}
              />
            ) : null}
            {tabIndex === '5' ? (
              <LgovTrfCfmButtonGroup
                handleLgovTrfConfirm={() =>
                  handleUpdateLgovTrf(shlSelectedData, 'A')
                }
                handleLgovTrfDeny={() =>
                  handleUpdateLgovTrf(shlSelectedData, 'D')
                }
                handleChange={handleRadioChange}
              />
            ) : null}
            {tabIndex === '6' ? (
              <LgovTrfntfReqButtonGroup
                handleLgovTrfCancel={() =>
                  handleUpdateLgovTrf(shlSelectedData, 'C')
                }
                handleChange={handleRadioChange}
              />
            ) : null}
        ...
        <TableContainer>
          <Table aria-labelledby="tableTitle" size={'medium'}>
            ...
            <EnhancedTableHead
              ...
              numSelected={shlSelectedData.length}
            />
            {/* 테이블의 내용부분  */}
            <TableBody>
              {!loading ? (
