...
const DataList = () => {
  ...
  useEffect(() => {
    if (shlInfo.shlSearchDone || flag) {
      fetchData()
    }
  }, [shlInfo.shlSearchDone, flag])
  ...
  const returnHeadCell = (tabValue: string): HeadCell[] => {
    return shlTabHeadCells[Number(tabValue) - 1]
  }
  ...
  return (
    <PageContainer title="단시간반복주유" description="단시간반복주유">
      {/* breadcrumb */}
      <Breadcrumb title="단시간반복주유" items={BCrumb} />
      {/* end breadcrumb */}
      ...
      <>
        {decisionBypeInfo.dbModalOpen ? <DecisionBypeModal dwNo="02" /> : null}
