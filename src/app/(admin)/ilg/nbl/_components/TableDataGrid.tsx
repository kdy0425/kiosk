...
const TableDataGrid: React.FC<ServerPaginationGridProps> = ({
  ...
  const nblInfo = useSelector((state: AppState) => state.nblInfo)
                ...
                {headCell.format === 'checkbox' ? (
                  <TableCell padding="checkbox">
                    ...
                    <CustomCheckbox
                      id="chkAll"
                      indeterminate={
                        nblSelectedData.length > 0 &&
                        nblSelectedData.length < rows.length
                      }
  ...
  const { nblSelectedData, nblExamResultData } = nblInfo
      ...
      <div className="data-grid-wrapper">
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center' }}>
            {tabIndex === '1' ? (
              <DoubtDelngButtonGroup
                ...
                handleBypeExamTrgetEach={() =>
                  handleBypeExamTrgetEach(nblSelectedData)
                }
              />
            ) : null}
            {tabIndex === '2' ? (
              <ExamTrgetButtonGroup
                handleCreateExamResult={() =>
                  handleCreateExamResult(nblSelectedData)
                }
                handleCreateLgovTr={() => handleCreateLgovTr(nblSelectedData)}
                handleCancelExamTrget={() =>
                  handleCancelExamTrget(nblSelectedData)
                }
              />
            ) : null}
            {tabIndex === '3' ? (
              <ExamResultButtonGroup
                handleCreateExaathr={() =>
                  handleCreateExaathr(nblExamResultData)
                }
                handleCancelExamResult={() =>
                  handleCancelExamResult(nblExamResultData)
                }
            ...
            {tabIndex === '4' ? (
              <ExaathrButtonGroup
                handleCancelExaathr={() => handleCancelExaathr(nblSelectedData)}
                handleChange={handleRadioChange}
              />
            ) : null}
            {tabIndex === '5' ? (
              <LgovTrfCfmButtonGroup
                handleLgovTrfConfirm={() =>
                  handleUpdateLgovTrf(nblSelectedData, 'A')
                }
                handleLgovTrfDeny={() =>
                  handleUpdateLgovTrf(nblSelectedData, 'D')
                }
                handleChange={handleRadioChange}
              />
            ) : null}
            {tabIndex === '6' ? (
              <LgovTrfntfReqButtonGroup
                handleLgovTrfCancel={() =>
                  handleUpdateLgovTrf(nblSelectedData, 'C')
                }
                handleChange={handleRadioChange}
              />
            ) : null}
        ...
        <TableContainer>
          <Table aria-labelledby="tableTitle" size={'medium'}>
            ...
            <EnhancedTableHead
              ...
              numSelected={nblSelectedData.length}
            />
            <TableBody>
              {!loading ? (
                rows.length > 0 ? (
