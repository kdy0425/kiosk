...
const DataList = () => {
  ...
  useEffect(() => {
    if (nblInfo.nblSearchDone || flag) {
      fetchData()
    }
  }, [nblInfo.nblSearchDone, flag])
  ...
  const returnHeadCell = (tabValue: string): HeadCell[] => {
    return nblTabHeadCells[Number(tabValue) - 1]
  }
  ...
  return (
    <PageContainer
      title="유효하지 않은 사업자의 의심주유"
      description="유효하지 않은 사업자의 의심주유"
    >
      {/* breadcrumb */}
      <Breadcrumb title="유효하지 않은 사업자의 의심주유" items={BCrumb} />
      {/* end breadcrumb */}
      ...
      <>
        {decisionBypeInfo.dbModalOpen ? <DecisionBypeModal dwNo="07" /> : null}
