...
// headCells
import { ilgDleeDoubtTransTxHC } from '@/utils/fsms/headCells' // 의심거래내역 탭 그리드
import { ilgDleeExamTargetTxHC } from '@/utils/fsms/headCells' // 조사대상내역 탭 그리드
...
const DataList = () => {
  ...
  // 지자체이첩 등록 모달 닫기
  const closeLocalTransModal = async () => {
  ...
  // 행정처분 등록 모달 닫기
  const closeAdminProcessModal = async () => {
  ...
  return (
    <PageContainer
      title="1일 120리터 초과충전"
      description="1일 120리터 초과충전"
    >
      {/* breadcrumb */}
      <Breadcrumb title="1일 120리터 초과충전" items={BCrumb} />
      {/* end breadcrumb */}
      ...
      <div style={{ marginTop: '-8px' }}>
        <BlankCard>
          <Box display={selectedTab === '0' ? 'block' : 'none'} sx={{ mb: 2 }}>
            ...
            <TabDataGrid
              headCells={ilgDleeDoubtTransTxHC} // 테이블 헤더 값
              rows={tabRows} // 목록 데이터
          ...
          <Box display={selectedTab === '1' ? 'block' : 'none'} sx={{ mb: 2 }}>
            ...
            <TabDataGrid
              headCells={ilgDleeExamTargetTxHC} // 테이블 헤더 값
              rows={tabRows} // 목록 데이터
              totalRows={tabTotalRows} // 총 로우 수
              loading={loading} // 로딩여부
              onPaginationModelChange={handlePaginationModelChange} // 페이지 , 사이즈 변경 핸들러 추가
