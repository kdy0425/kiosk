...
// headCells
import { ilgDmmmareDtlTxHC } from '@/utils/fsms/headCells' // 차량별 거래내역 상세 그리드 (하단)
import { ilgDmmmareDoubtTransTxHC } from '@/utils/fsms/headCells' // 의심거래내역 탭 그리드
import { ilgDmmmareExamTargetTxHC } from '@/utils/fsms/headCells' // 조사대상내역 탭 그리드
...
const DataList = () => {
  ...
  const fetchTab = async () => {
    ...
    try {
      ...
      if (response && response.resultType === 'success' && response.data) {
        ...
        if (selectedTab === '0' && response.data.totalElements > 0) {
  ...
  const fetchDtl = async () => {
    try {
      ...
      const endpoint: string =
        `/fsm${BCrumb[BCrumb.length - 1].to}/tx/getAllDay4DoubtReptDtlList?page=${dtlParams.page}&size=${dtlParams.size}` +
  ...
  const dtlExcelDownload = async () => {
    const endpoint: string =
      `/fsm${BCrumb[BCrumb.length - 1].to}/tx/getExcelAllDay4DoubtReptDtlList?` +
      `${'&dlngYm=' + dtlParams.dlngYm}` +
      `${'&ctpvCd=' + dtlParams.ctpvCd}` +
      `${'&locgovCd=' + dtlParams.locgovCd}` +
      `${'&brno=' + dtlParams.brno}` +
...
await  getExcelFile(
  ...
  const handelDtlListPagenation = (page: number, pageSize: number) => {
    setDtlParams((prev) => ({
      ...prev,
      page: page,
      size: pageSize,
    ...
    >
      ...
      <div style={{ marginTop: '-8px' }}>
        <BlankCard>
          <Box display={selectedTab === '0' ? 'block' : 'none'} sx={{ mb: 2 }}>
            ...
            <TabDataGrid
              headCells={ilgDmmmareDoubtTransTxHC} // 테이블 헤더 값
              rows={tabRows} // 목록 데이터
          ...
          <Box display={selectedTab === '1' ? 'block' : 'none'} sx={{ mb: 2 }}>
            ...
            <TabDataGrid
              headCells={ilgDmmmareExamTargetTxHC} // 테이블 헤더 값
              rows={tabRows} // 목록 데이터
              totalRows={tabTotalRows} // 총 로우 수
              loading={tabLoading} // 로딩여부
              onRowClick={handleRowClick} // 행 클릭 핸들러 추가
      ...
      <div>
        {dtlShowFlag && (
          ...
          >
            <DtlDataGrid
              headCells={ilgDmmmareDtlTxHC} // 테이블 헤더 값
              rows={dtlRows} // 목록 데이터
