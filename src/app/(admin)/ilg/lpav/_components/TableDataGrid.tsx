...
// 테이블 caption
const tableCaption: string = '주유패턴이상차량 의심거래내역'
...
const TableDataGrid: React.FC<ServerPaginationGridProps> = ({
  ...
  function EnhancedTableHead(props: Readonly<EnhancedTableProps>) {
    ...
    const { lpavSelectedData } = lpavInfo
    return (
      <TableHead>
        <TableRow key={'thRow'}>
              ...
              <React.Fragment key={'th' + headCell.id}>
                {headCell.format === 'checkbox' ? (
                  <TableCell padding="checkbox">
                    ...
                    <CustomCheckbox
                      indeterminate={
                        lpavSelectedData.length > 0 &&
                        lpavSelectedData.length < rows.length
                      }
                      id="chkAll"
  ...
  const { lpavSelectedData, lpavExamResultData } = lpavInfo
  return (
      ...
      <div className="data-grid-wrapper">
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center' }}>
            {tabIndex === '1' ? <DoubtDelngButtonGroup dwNo="01" /> : null}
            {tabIndex === '2' ? (
              <ExamTrgetButtonGroup
                handleCreateExamResult={() =>
                  handleCreateExamResult(lpavSelectedData)
                }
                handleCreateLgovTr={() => handleCreateLgovTr(lpavSelectedData)}
                handleCancelExamTrget={() =>
                  handleCancelExamTrget(lpavSelectedData)
                }
              />
            ) : null}
            {tabIndex === '3' ? (
              <ExamResultButtonGroup
                handleCreateExaathr={() =>
                  handleCreateExaathr(lpavExamResultData)
                }
                handleCancelExamResult={() =>
                  handleCancelExamResult(lpavExamResultData)
                }
            ...
            {tabIndex === '4' ? (
              <ExaathrButtonGroup
                handleCancelExaathr={() =>
                  handleCancelExaathr(lpavSelectedData)
                }
                handleChange={handleRadioChange}
              />
            ) : null}
            {tabIndex === '5' ? (
              <LgovTrfCfmButtonGroup
                handleLgovTrfConfirm={() =>
                  handleUpdateLgovTrf(lpavSelectedData, 'A')
                }
                handleLgovTrfDeny={() =>
                  handleUpdateLgovTrf(lpavSelectedData, 'D')
                }
                handleChange={handleRadioChange}
              />
            ) : null}
            {tabIndex === '6' ? (
              <LgovTrfntfReqButtonGroup
                handleLgovTrfCancel={() =>
                  handleUpdateLgovTrf(lpavSelectedData, 'C')
                }
                handleChange={handleRadioChange}
              />
            ) : null}
        ...
        <TableContainer>
          <Table aria-labelledby="tableTitle" size={'medium'}>
            ...
            <EnhancedTableHead
              ...
              numSelected={lpavSelectedData.length}
            />
            {/* 테이블의 내용부분  */}
            <TableBody>
              {!loading ? (
