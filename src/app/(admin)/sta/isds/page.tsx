...
import { arSA } from '@mui/material/locale'
  ...
  const DataList = () => {
    ...
    useEffect(() => {
      ...
      // },
        ...
        header.filter((item) => item.mapId !== "_SUB") .forEach((item) => headCells.push({
          id: item.mapId, // 열의 ID
          numeric: false, // 숫자 여부 (필요에 따라 수정)
          disablePadding: false,
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        let endpoint: string =
          `/fsm/sta/isds/getAllInstcSpldmdDisclSttus?` +
        ...
        if (response && response.resultType === 'success' && response.data) {
          ...
          setRows(
            response.data.data.map((row: any) => ({
              ...row,
              _SUB : row._SUB == null || row._SUB == '0' ? ' 0' :  row._SUB,
              AAAA: row.AAA === row.AAAA ? null : row.AAAA, // AAA와 AAAA가 같으면 AAAA를 null로 설정
            }))
          );
  ...
  //   }
    ...
    return (
      <PageContainer
        title="부정수급적발 현황"
        description="부정수급적발 현황"
      >
        {/* breadcrumb */}
        <Breadcrumb title="부정수급적발 현황" items={BCrumb} />
        {/* end breadcrumb */}
