...
import { staLbfpsHC } from '@/utils/fsms/headCells'
...
const DataList = () => {
  ...
  const fetchData = async () => {
    if (searchValidation()) {
      ...
      try {
        ...
        let endpoint: string =
          `/fsm/sta/lbfps/cm/getAllByliinFsmPymntSttus?` +
  ...
  const excelDownload = async () => {
    ...
    if (searchValidation()) {
      let endpoint: string =
        `/fsm/sta/bylfps/cm/getExcelByliinFsmPymntSttus?` +
        `${params.strCrtrYm ? '&strCrtrYm=' + params.strCrtrYm.replaceAll('-', '') : ''}` +
        `${params.endCrtrYm ? '&endCrtrYm=' + params.endCrtrYm.replaceAll('-', '') : ''}` +
        `${params.lcnsTpbizCd ? '&lcnsTpbizCd=' + params.lcnsTpbizCd : ''}` +
        `${prdAdupYn ? '&groupYn=Y' : '&groupYn=N'}`
  ...
  return (
    <PageContainer
      title="면허업종별 유가보조금 지급현황"
      description="면허업종별 유가보조금 지급현황"
    >
      {/* breadcrumb */}
      <Breadcrumb title="면허업종별 유가보조금 지급현황" items={BCrumb} />
      {/* end breadcrumb */}
