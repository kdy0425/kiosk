...
import { getCodesByGroupNm } from '@/utils/fsms/common/code/getCode'
  ...
  const DataList = () => {
    ...
    // 화물 버스만 조회하기 위한 selectItem 
    const [srchGbCodes, setSrchGbCodes] = useState<SelectItem[]>([]) // 관할관청 코드
    ...
    useEffect(() => {
      ...
      //사업자 구분
      getCodesByGroupNm('801').then((res) => {
        if (res) {
          res.map((code: any) => {
            let item: SelectItem = {
              label: code['cdKornNm'],
  ...
  //   }
    ...
    return (
      <PageContainer
        title="RFID발급실적"
        description="RFID발급실적"
      >
        {/* breadcrumb */}
        <Breadcrumb title="RFID발급실적" items={BCrumb} />
        {/* end breadcrumb */}
        ...
        <Box component="form" sx={{ mb: 2 }}>
          <Box className="sch-filter-box">
            <div className="filter-form">
              ...
              <div className="form-group">
                <CustomFormLabel className="input-label-display" htmlFor="ft-srchGb">
                  구분
                </CustomFormLabel>
                <select
                  id="ft-srchGb"
