...
import { symBccHC } from '@/utils/fsms/headCells'
...
const DataList = () => {
  ...
  symBccHC[0].button = {
    ...
    onClick: async (row:Row, event: any) => {
      ...
      let endpoint : string = `/fsm/sym/bcc/bs/excuteBusCardCntc?procNm=${row.procNm}`;
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/sym/bcc/bs/getAllBusCardCntc?page=${params.page}&size=${params.size}` +
  ...
  return (
    <PageContainer
      title="버스 카드협약사 연계관리"
      description="버스 카드협약사 연계관리"
    >
      {/* breadcrumb */}
      <Breadcrumb title="버스 카드협약사 연계관리" items={BCrumb} />
      {/* end breadcrumb */}
        ...
        <Box className="table-bottom-button-group">
          <div className="button-right-align">
            ...
            <ModalContent
              ...
              title="연계처리이력"
              url="/fsm/sym/bcc/bs/getAllBusCardCntcHst"
              open={open}
              row={selectedRow}
            />
          </div>
