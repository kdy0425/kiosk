...
// 테이블 caption
const tableCaption :string = '화물 카드협약사 연계관리 목록'
...
interface ServerPaginationGridProps {
  ...
  onClickButton: (procNm: string, e: React.MouseEvent<unknown>) => any
            ...
            <TableBody>
              {!loading ? 
                rows.length > 0 ? rows.map((row: any, index) => {
                  return (
                    <TableRow hover key={'tr'+index} onClick={() => onRowClick(row.procNm)}>
                      <TableCell>
                        <Button onClick={(e) => onClickButton(row.procNm, e)} >재실행</Button>
                      </TableCell>
                      <TableCell>
                        {row.procNm}
                      </TableCell>
