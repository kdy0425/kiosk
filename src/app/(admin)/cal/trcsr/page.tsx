...
import { calTrcsrCsbySbsidyRqestTrHc } from '@/utils/fsms/headCells'
...
const DataList = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/cal/trcsr/tr/getAllCsbySbsidy?page=${params.page}&size=${params.size}` +
        `${params.searchStDate ? '&clclnStartYm=' + formatDate(params.searchStDate) : ''}` +
        `${params.searchEdDate ? '&clclnEndYm=' + formatDate(params.searchEdDate) : ''}` +
  ...
  const excelDownload = async () => {
    ...
    let endpoint: string =
      `/fsm/cal/trcsr/tr/getExcelCsbySbsidy?` +
      `${params.searchStDate ? '&clclnStartYm=' + formatDate(params.searchStDate) : ''}` +
      `${params.searchEdDate ? '&clclnEndYm=' + formatDate(params.searchEdDate) : ''}` +
      `${params.ctpvCd ? '&ctpvCd=' + params.ctpvCd : ''}` +
      `${params.locgovCd ? '&locgovCd=' + params.locgovCd : ''}` +
  ...
  return (
    <PageContainer
      title="건별청구내역조회(화물)"
      description="건별청구내역조회(화물)"
    >
      {/* breadcrumb */}
      <Breadcrumb title="건별청구내역조회(화물)" items={BCrumb} />
      {/* end breadcrumb */}
          ...
          <div className="filter-form">
            <div className="form-group">
              <CustomFormLabel
                ...
                htmlFor="sch-crdco"
              >
                <span className="required-text">*</span>카드사 구분
              </CustomFormLabel>
              <CommSelect
                cdGroupNm="023"
            ...
            <CommTextField type={'vhclNo'} handleChange={handleSearchChange} />
          </div>
        </Box>
      ...
      <Box>
        <TableDataGrid
          headCells={calTrcsrCsbySbsidyRqestTrHc} // 테이블 헤더 값
          rows={rows} // 목록 데이터
