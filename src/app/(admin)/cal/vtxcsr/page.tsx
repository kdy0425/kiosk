...
// 헤더셀
import { calVtxcsrCalHC } from '@/utils/fsms/headCells'
  ...
  return (
    <TableHead>
      <TableRow>
        ...
        <TableCell colSpan={6}>정상거래</TableCell>
        <TableCell colSpan={6}>취소거래</TableCell>
      </TableRow>
      <TableRow>
...
const DataList = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/cal/vtxcsr/tx/getAllByveSbsidyRqest?page=${params.page}&size=${params.size}` +
  ...
  const excelDownload = async () => {
    ...
    let endpoint: string =
      `/fsm/cal/vtxcsr/tx/getExcelAllByveSbsidyRqest?page=0&size=9999` +
      `${params.locgovCd ? '&clclnLocgovCd=' + params.locgovCd : ''}` +
      `${params.bgngDt ? '&bgngDt=' + formatDate(params.bgngDt) : ''}` +
      `${params.endDt ? '&endDt=' + formatDate(params.endDt) : ''}` +
      `${params.koiCd ? '&koiCd=' + params.koiCd : ''}` +
    ...
    >
      ...
      <Box component="form" onSubmit={handleAdvancedSearch} sx={{ mb: 2 }}>
        <Box className="sch-filter-box">
          ...
          <div className="filter-form">
            ...
            <div className="form-group">
              <CustomFormLabel className="input-label-display" htmlFor="ft-vhclNo">차량번호</CustomFormLabel>
              <CustomTextField 
                type="text"
                id="ft-vhclNo"
                name="vhclNo"
      ...
      <Box>
        <TableDataGrid
          ...
          headCells={calVtxcsrCalHC} // 테이블 헤더 값
          rows={rows} // 목록 데이터
