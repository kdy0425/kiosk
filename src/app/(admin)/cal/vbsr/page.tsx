...
import { calVbsrBsHC } from '@/utils/fsms/headCells'
import {
  CtpvSelect,
  LocgovSelect,
  CommSelect,
...
const DataList = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/cal/vbsr/bs/getAllByveSbsidyRqestCur?page=${params.page}&size=${params.size}` +
  ...
  const excelDownload = async () => {
    ...
    let endpoint: string =
      `/fsm/cal/vbsr/bs/getExcelByveSbsidyRqestCur?` +
      `${params.searchStDate ? '&bgngDt=' + params.searchStDate.replace(/-/g, '') : ''}` +
      `${params.searchEdDate ? '&endDt=' + params.searchEdDate.replace(/-/g, '') : ''}` +
      `${params.ctpvCd ? '&ctpvCd=' + params.ctpvCd : ''}` +
      `${params.locgovCd ? '&locgovCd=' + params.locgovCd : ''}` +
  ...
  return (
    <PageContainer title="차량별청구현황" description="차량별청구현황">
      {/* breadcrumb */}
      ...
      <Box>
        <TableDataGrid
          headCells={calVbsrBsHC} // 테이블 헤더 값
          rows={rows} // 목록 데이터
