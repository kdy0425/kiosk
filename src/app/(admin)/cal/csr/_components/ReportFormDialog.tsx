...
import { CustomFormLabel, CustomRadio, FormControlLabel, Grid, RadioGroup } from '@/utils/fsms/fsm/mui-imports';
...
// Fetch를 통해 데이터 갱신
  const fetchOutputData = async () => {
    ...
    try {
      ...
      const url = radio === 'S' ? `/fsm/cal/csr/tr/getAllCtprvnSbsidyReport?`: `/fsm/cal/csr/tr/getEachCtprvnSbsidyReport?`

      let endpoint: string =
        url +
        `${crdcoCd ? '&crdcoCd=' + crdcoCd : ''}` +
        `${selectedRow.clclnYm ? '&clclnYm=' + selectedRow.clclnYm : ''}` +
        `${selectedRow.clclnLocgovCd ? '&clclnLocgovCd=' + selectedRow.clclnLocgovCd : ''}`
      ...
      if (response && response.resultType === 'success' && response.data) {
        ...
        if(response.data.content[0] == undefined){
            ...
            //const jsonData = radio === 'S' ? { allSbsidyTr: row} : { eachSbsidyTr: row }
            // handleReport(
            // radio === 'S' ? 'allCtprvnSbsidyTr' : 'eachCtprvnSbsidyTr',
            // JSON.stringify(jsonData),
            // )
            const jsonData = { eachCtprvnSbsidyTr: row }

            var crfName;
            var crfData;
            if (Array.isArray(row) && row.length === 1) {
                crfName = radio === 'S' ? 'allCtprvnSbsidyTr' : 'eachCtprvnSbsidyTr';
                crfData = JSON.stringify(jsonData);
            }else{
                crfName = radio === 'S' ? 'allCardCtprvnSbsidyTr' : 'allEachCtprvnSbsidyTr';
                crfData = JSON.stringify(jsonData)
            }
        ...
        <DialogContent>
            <Box className="table-bottom-button-group">
                ...
                <div className="button-right-align">
                    <Button variant="contained" color="success" onClick={fetchOutputData}>출력</Button>
