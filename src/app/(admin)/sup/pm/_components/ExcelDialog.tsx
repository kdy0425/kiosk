...
export default function ExcelDialog(props: ExcelDialogProps) {
  ...
  const fetchFilesDown = async () => {
    try {
      let endpoint: string = `/fsm/sup/pm/cm/getExcelFormmPrpdvsMng`
      ...
      link.href = url
      link.setAttribute('download', '안분관리_엑셀업로드.xlsx')
  ...
  const fetchFilesUpload = async (): Promise<boolean> => {
    ...
    try {
      ...
      let endpoint: string = `/fsm/sup/pm/cm/insertExcelPrpdvsMng`
