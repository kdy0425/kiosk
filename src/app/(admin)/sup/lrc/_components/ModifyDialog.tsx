...
export default function ModifyDialog(props: ModifyDialogProps) {
  ...
  const fetchFilesDown = async (files: CustomFile) => {
    ...
    try {
      let endpoint: string =
        `/fsm/sup/lrc/file/` + files.bbscttSn + `/` + files.atchSn
  ...
  const fetchFilesDelete = async (files: CustomFile): Promise<boolean> => {
    ...
    try {
      let endpoint: string =
        `/fsm/sup/lrc/file/deleteBoardFile?` +
  ...
  const handleSave = async () => {
    try {
      ...
      const endpoint = `/fsm/sup/lrc/updateBoardDtl`
      ...
      if (response?.resultType === 'success') {
        alert('소송/심판 사례 데이터가 수정되었습니다.')
        reloadFunc?.()
        handleClose()
      } else {
        console.error('Response fail:', response)
        alert(
          `소송/심판 사례 데이터 수정 응답이 성공이 아닙니다. (${response?.message || 'Unknown Error'})`,
        )
        reloadFunc?.()
        handleClose()
      }
      ...
      alert(`Error : 소송/심판 사례 데이터 수정에 실패했습니다. `)
      handleClose()
    }
  }
  ...
  const handleCommSave = async () => {
    try {
      const endpoint = `/fsm/sup/lrc/cmnt/createBoardCmnt`
  ...
  const handleCommDelete = async (comments: CommentList): Promise<boolean> => {
    ...
    try {
      let endpoint: string =
        `/fsm/sup/lrc/cmnt/deleteBoardCmnt?` +
                    ...
                    <TableBody>
                      <TableRow>
                        ...
                        <TableCell style={{ textAlign: 'left' }}>
                          <CustomFormLabel className="input-label-none" htmlFor="modal-commentCn">댓글내용</CustomFormLabel>
