...
const RegisterModalForm = (props: ModalFormProps) => {
  ...
  useEffect(() => {
    const fetchCodes = async () => {
      ...
      const notiRes = await getCodesByGroupNm('120');
  ...
  const createNotice = async () => {
    try {
      ...
      // 사용자 확인
      const userConfirm = confirm('소송/심판 사례 데이터를 등록하시겠습니까?');
      if (!userConfirm) {
        return;
      }

      const endpoint = '/fsm/sup/lrc/createBoardDtl/';
      ...
      if (response?.resultType === 'success') {
        ...
        alert(`소송/심판 사례 데이터 등록 응답이 성공이 아닙니다. (${response?.message || 'Unknown Error'})`);
        handleClose();
        reloadFunc?.();
      }
    } catch (error) {
      ...
      alert(`Error : 소송/심판 사례 데이터 등록에 실패했습니다. `)
      handleClose();
