...
export interface Row {
  ...
  leadCnCd?: string // 머릿글내용코드    // 소송/심판 사례 코드임.
...
const DataList = () => {
  ...
  useEffect(() => {
    ...
    //구분
    getCodesByGroupNm('120').then((res) => {
      if (res) {
        res.map((code: any) => {
          let item: SelectItem = {
            label: code['cdKornNm'],
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/sup/lrc?page=${params.page}&size=${params.size}` +
  ...
  const fetchDetailData = async (row: Row) => {
    try {
      // 검색 조건에 맞는 endpoint 생성
      let endpoint: string = `/fsm/sup/lrc/` + row?.bbscttSn
  ...
  const handleSortModelChange = (sort: string) => {
    ...
    fetchData() // 정렬 기준이 변경되었으므로 데이터 재조회
  }
  ...
  return (
    <PageContainer title="소송/심판 사례" description="소송/심판 사례">
      {/* breadcrumb */}
      <Breadcrumb title="소송/심판 사례" items={BCrumb} />
      {/* end breadcrumb */}
