...
const ModifyModalForm = (props: ModalFormProps) => {
  const { reloadFunc, selectedRow, open, handleDetailCloseModal } = props
  ...
  const [params, setParams] = useState<Row>(selectedRow)
  ...
  useEffect(() => {
    setParams((prev) => ({
      ...prev,
      crtrYm:
        selectedRow.crtrYm.substring(0, 4) +
        '-' +
        selectedRow.crtrYm.substring(4, 6),
      ctpvCd: selectedRow.locgovCd.substring(0, 2),
      carmdlCd: selectedRow.carmdlCd,
    }))
    setIsLocgovCdAll(true)
  }, [])
  ...
  const deleteCode = async () => {
    let endpoint: string = `/fsm/sup/pmr/cm/deletePerfMng`
    ...
    const userConfirm = confirm(
      params.crtrYm + ' 지급실적정보를 삭제하시겠습니까?',
    )
    if (userConfirm) {
  ...
  const handleSave = async () => {
    ...
    // 사용자 확인
    const userConfirm = confirm('지급실적정보를 수정하시겠습니까?')
    if (userConfirm) {
      const endpoint: string = `/fsm/sup/pmr/cm/updatePerfMng`
      ...
      const jsonData = {
        ...
        cardGiveAmt: parseFloat(params.cardGiveAmt.replaceAll(',', '')),
        cardGiveAog: parseFloat(params.cardGiveAog.replaceAll(',', '')),
        cardGiveVhclCnt: parseFloat(params.cardGiveVhclCnt.replaceAll(',', '')),
        docmntAplyGiveAmt: parseFloat(
          params.docmntAplyGiveAmt.replaceAll(',', ''),
        ),
        docmntAplyGiveLbrctQty: parseFloat(
          params.docmntAplyGiveLbrctQty.replaceAll(',', ''),
        ),
        docmntAplyGiveVhclCnt: parseFloat(
          params.docmntAplyGiveVhclCnt.replaceAll(',', ''),
        ),
      }
      ...
      if (response?.resultType === 'success') {
        ...
        alert(
          `지급실적정보 데이터 수정 응답이 성공이 아닙니다. (${response?.message || 'Unknown Error'})`,
        )
        // handleClose();
        // reloadFunc?.();
      }
            ...
            >
              <TableContainer style={{ margin: '0 0 -1em 0' }}>
                ...
                <div className="contents-explanation">
                  <div className="contents-explanation-inner">
                    <div className="contents-explanation-text">
                      청구내역조회시 기준월에 청구된 내역이 자동으로 매핑됩니다.
                    </div>
                  </div>
                </div>
              </TableContainer>
            </BlankCard>
            <BlankCard className="contents-card fit" title="서면신청 지급실적">
              <TableContainer style={{ margin: '0 0 -1em 0' }}>
                <Table
                  className="table table-bordered"
                  style={{ tableLayout: 'fixed', width: '100%' }}
