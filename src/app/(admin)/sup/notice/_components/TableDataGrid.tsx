...
import CommTablePagination from '@/components/tables/CommTablePagination'
...
const TableDataGrid: React.FC<ServerPaginationGridProps> = ({
  ...
  const handleChangePage = (event: unknown, newPage: number) => {
    onPaginationModelChange(newPage-1,pageable.pageSize)
  };
  ...
  const handleRowClick = (event: React.FormEvent, row :Row) => {
  ...
  {!loading ? (
    rows.length > 0 ? (
      <>
              {rows
                .filter((row) => row.popupNtcYn === 'Y')
                .map((row, index) => (
                  <TableRow hover onClick={(e) => handleRowClick(e,row)} key={`highlighted-tr${index}`}  sx={{ cursor: 'pointer' }}>
                    <TableCell sx={{ width: '10%'}} >
                    <span style={{ color: 'orange' }}>{row.bbscttSn ?? ''}</span>
                    </TableCell>
                    <TableCell sx={{ width: '60%', color: 'orange' }}>
                    <span style={{ color: 'orange' }}> {row.oriTtl ?? ''}</span>
                    </TableCell>
                    <TableCell sx={{ width: '15%', color: 'orange' }}>
                    <span style={{ color: 'orange' }}> {row.inqCnt ?? ''}</span>
                    </TableCell>
                    <TableCell sx={{ width: '15%', color: 'orange' }}>
                    <span style={{ color: 'orange' }}> {formatDate(row.regDt)}</span>
                    </TableCell>
                  </TableRow>
                ))}
              {rows
                .filter((row) => row.popupNtcYn !== 'Y')
                .map((row, index) => (
                  <TableRow hover onClick={() => onRowClick(row)} key={`tr${index}`} sx={{ cursor: 'pointer' }}>
                    <TableCell sx={{ width: '10%' }}>
                      {row.bbscttSn ?? ''}
                    </TableCell>
            ...
            <TableRow key={'tr0'}>
              <TableCell colSpan={4} className='td-center'>
