...
const RegisterModalForm = (props: ModalFormProps) => {
  ...
  const createNotice = async () => {
    try {
      ...
      // 사용자 확인
      const userConfirm = confirm('공지사항을 등록하시겠습니까?');
      if (!userConfirm) {
        return;
      }

      const endpoint = '/fsm/sup/notice/createBoardDtl/';
      ...
      if (response?.resultType === 'success') {
        ...
        alert(`공지사항 등록 응답이 성공이 아닙니다. (${response?.message || 'Unknown Error'})`);
        handleClose();
        reloadFunc?.();
      }
    } catch (error) {
            ...
            <Table  style={{ tableLayout: 'fixed', width: '100%' }}>
              <TableBody>
                <TableRow>
                  ...
                  <span className="required-text" >*</span>공지구분
                  </TableCell>
                ...
                <TableRow>
                <TableCell style={{ width: '150px', verticalAlign: 'middle' }}>
                  <span className="required-text">*</span>팝업여부
                </TableCell>
