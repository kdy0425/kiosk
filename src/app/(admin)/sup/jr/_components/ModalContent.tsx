...
const RegisterModalForm = (props: ModalFormProps) => {
  ...
  useEffect(() => {
    const fetchCodes = async () => {
      ...
      const notiRes = await getCodesByGroupNm('119');
  ...
  const createNotice = async () => {
    try {
      ...
      // 사용자 확인
      const userConfirm = confirm('업무요청 데이터를 등록하시겠습니까?');
      if (!userConfirm) {
        return;
      }

      const endpoint = '/fsm/sup/jr/createBoardDtl/';
      ...
      const jsonData = {
        ...
        inclLocgovCd: authInfo && 'locgovCd' in authInfo ? authInfo.locgovCd : '',
      ...
      if (response?.resultType === 'success') {
        ...
        alert(`업무요청 데이터 등록 응답이 성공이 아닙니다. (${response?.message || 'Unknown Error'})`);
        handleClose();
        reloadFunc?.();
      }
    } catch (error) {
      ...
      alert(`Error : 업무요청 데이터 등록에 실패했습니다. `)
      handleClose();
            ...
            <Table  style={{ tableLayout: 'fixed', width: '100%' }}>
              <TableBody>
                <TableRow>
                  ...
                  <CustomFormLabel className="input-label-none" htmlFor="modal-select-comCdYn">구분</CustomFormLabel>
                    <select className="custom-default-select"
