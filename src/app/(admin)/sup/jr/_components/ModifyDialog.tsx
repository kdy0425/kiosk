...
export default function ModifyDialog(props: ModifyDialogProps) {
  ...
  const deleteCode = async () => {
    ...
    const userConfirm = confirm('업무요청 데이터를 삭제하시겠습니까?')
  ...
  const fetchFilesDown = async (files: CustomFile) => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/sup/jr/file/` + files.bbscttSn + `/` + files.atchSn
  ...
  const fetchFilesDelete = async (files: CustomFile): Promise<boolean> => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/sup/jr/file/deleteBoardFile?` +
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const endpoint = `/fsm/sup/jr/updateBoardDtl`
      ...
      if (response?.resultType === 'success') {
        alert('업무요청 데이터가 수정되었습니다.')
        reloadFunc?.()
        handleClose()
      } else {
        console.error('Response fail:', response)
        alert(
          `업무요청 데이터 수정 응답이 성공이 아닙니다. (${response?.message || 'Unknown Error'})`,
        )
        reloadFunc?.()
        handleClose()
      }
      ...
      alert(`Error : 업무요청 데이터 수정에 실패했습니다. `)
      handleClose()
    } finally {
      setLoadingBackdrop(false)
    }
  ...
  const handleCommSave = async () => {
    ...
    try {
      ...
      const endpoint = `/fsm/sup/jr/cmnt/createBoardCmnt`
      ...
      if (response?.resultType === 'success') {
        alert('업무요청 댓글이 등록되었습니다.')
        reloadFunc?.()
        // handleClose();
      } else {
        console.error('Response fail:', response)
        alert(
          `업무요청 댓글 등록 중 오류가 발생하였습니다.(${response?.message || 'Unknown Error'})`,
        )
        // reloadFunc?.();
        // handleClose();
      }
      ...
      console.error('Error during update:', error)
      alert(`Error : 업무요청 댓글 등록에 실패하였습니다. `)
      // handleClose();
    } finally {
      setLoadingBackdrop(false)
    }
  ...
  const handleCommDelete = async (comments: CommentList): Promise<boolean> => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/sup/jr/cmnt/deleteBoardCmnt?` +
      ...
      if (response && response.resultType === 'success') {
        alert('업무요청 댓글이 삭제되었습니다.')
        reloadFunc?.()
        return true
      } else {
        alert('업무요청 댓글 삭제 중 오류가 발생했습니다.')
        return false
      }
    } catch (error) {
      // 에러시
