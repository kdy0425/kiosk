...
export interface Row {
  ...
  leadCnCd?: string // 머릿글내용코드    // 업무요청 코드임.
...
const DataList = () => {
  ...
  const [params, setParams] = useState<listSearchObj>({
    ...
    search: allParams.search ?? 'ttl', // 종류
    searchValue: allParams.searchValue ?? '', // 검색어
    searchSelect: allParams.searchSelect ?? '', // 종류
    searchStDate: allParams.searchStDate ?? '', // 시작일
    searchEdDate: allParams.searchEdDate ?? '', // 종료일
    work: allParams.work ?? '00', // 업무구분
  })
  //
  ...
  const handleReload = async () => {
    ...
    setFlag(!flag)
    if (selectedRow) {
      await fetchDetailData(selectedRow)
    }
  }
  ...
  useEffect(() => {
    ...
    //구분
    getCodesByGroupNm('119').then((res) => {
      if (res) {
        res.map((code: any) => {
          let item: SelectItem = {
            label: code['cdKornNm'],
    ...
    const dateRange = getDateRange('d', 90)
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/sup/jr?page=${params.page}&size=${params.size}` +
        ...
        `${!isAdmin ? '&locgovCd=' + userInfo.locgovCd : ''}`
  ...
  const fetchDetailData = async (row: Row) => {
    try {
      // 검색 조건에 맞는 endpoint 생성
      let endpoint: string = `/fsm/sup/jr/` + row?.bbscttSn

      // 날짜, 구분, 업무구분, 검색종류, 검색어 확실해지면 입력할 것
  ...
  return (
    <PageContainer title="업무요청" description="업무요청">
      {/* breadcrumb */}
      <Breadcrumb title="업무요청" items={BCrumb} />
      {/* end breadcrumb */}
