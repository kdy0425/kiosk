...
const DataList = () => {
  ...
  const [leadCnCode, setLeadCnCode] = useState<SelectItem[]>([]) //        자료구분 코드
  ...
  useEffect(() => {
    ...
    //자료구분
    getCodesByGroupNm('113').then((res) => {
      if (res) {
        res.map((code: any) => {
          let item: SelectItem = {
            label: code['cdKornNm'],
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/sup/recroom?page=${params.page}&size=${params.size}` +
      ...
      // 날짜, 자료구분, 업무구분, 검색종류, 검색어 확실해지면 입력할 것
  ...
  const fetchDetailData = async (row: Row) => {
    try {
      // 검색 조건에 맞는 endpoint 생성
      let endpoint: string = `/fsm/sup/recroom/` + row?.bbscttSn
  ...
  return (
    <PageContainer title="공지사항" description="자료실">
      {/* breadcrumb */}
      <Breadcrumb title="자료실" items={BCrumb} />
      {/* end breadcrumb */}
