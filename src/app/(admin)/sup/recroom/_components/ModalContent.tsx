...
const RegisterModalForm = (props: ModalFormProps) => {
  ...
  const [leadCnCode, setNotiCode] = useState<SelectItem[]>([])  //        자료구분 코드 
  ...
  const handleClose = () => {
    setParams({
      ...
      leadCnCd: "00", // 머릿글내용코드 = 머릿글 자료구분코드
      ttl: "", // 게시글제목
      cn: "", // 게시글내용
      popupNtcYn: "N", // 팝업공지여부
      popupBgngYmd: "", // 팝업시작일자
  ...
  useEffect(() => {
    const fetchCodes = async () => {
      ...
      const notiRes = await getCodesByGroupNm('113');
  ...
  const createNotice = async () => {
    try {
      ...
      // 사용자 확인
      const userConfirm = confirm('자료실 데이터를 등록하시겠습니까?');
      if (!userConfirm) {
        return;
      }

      const endpoint = '/fsm/sup/recroom/createBoardDtl/';

      // FormData 생성
      const jsonData = {
        //bbsSn: '3', // 게시판일련번호
        relateTaskSeCd: params.relateTaskSeCd, // 관련업무구분코드
        leadCnCd: params.leadCnCd, // 머릿글내용코드 = 머릿글 자료구분코드
      ...
      if (response?.resultType === 'success') {
        ...
        alert(`자료실 데이터 등록 응답이 성공이 아닙니다. (${response?.message || 'Unknown Error'})`);
        handleClose();
        reloadFunc?.();
      }
    } catch (error) {
      ...
      alert(`Error : 자료실 데이터 등록에 실패했습니다. `)
      handleClose();
            ...
            <Table  style={{ tableLayout: 'fixed', width: '100%' }}>
              <TableBody>
                <TableRow>
                  ...
                  <span className="required-text" >*</span>자료구분
                  </TableCell>
                  <TableCell style={{ width: 'calc(50% - 150px)', textAlign: 'left' }}>
                  <CustomFormLabel className="input-label-none" htmlFor="modal-select-leadCnCd">공지구분</CustomFormLabel>
                  ...
                  <TableCell style={{ width: 'calc(50% - 150px)', textAlign: 'left' }}>
                  <CustomFormLabel className="input-label-none" htmlFor="modal-select-relateTaskSeCd">업무구분</CustomFormLabel>
                  <select className="custom-default-select" style={{ width: '100%' }}
