...
export default function ModifyDialog(props: ModifyDialogProps) {
  ...
  const deleteCode = async () => {
    ...
    const userConfirm = confirm('자료실 데이터를 삭제하시겠습니까?')
    if (!userConfirm) return;

    try{
      setLoadingBackdrop(true)

      let endpoint: string = `/fsm/sup/recroom/deleteBoardDtl?` +
  ...
  const fetchFilesDown = async (files: CustomFile) => {
    ...
    try {
      ...
      let endpoint: string = `/fsm/sup/recroom/file/` + files.bbscttSn + `/` + files.atchSn
  ...
  const fetchFilesDelete = async (files: CustomFile): Promise<boolean> => {
    ...
    try {
      ...
      let endpoint: string = `/fsm/sup/recroom/file/deleteBoardFile?` +
  ...
  const handleSave = async () => {
    ...
    // 필수 값 체크
    if (!formData.leadCnCd || !formData.relateTaskSeCd ||
        !formData.oriTtl || !formData.cn
    ) {
    ...
    try {
      ...
      const endpoint = `/fsm/sup/recroom/updateBoardDtl`
      ...
      if (response?.resultType === 'success') {
        alert('자료실 데이터가 수정되었습니다.')
        reloadFunc?.()
        handleClose()
      } else {
        console.error('Response fail:', response)
        alert(
          `자료실 데이터 수정 응답이 성공이 아닙니다. (${response?.message || 'Unknown Error'})`,
        )
        reloadFunc?.()
        handleClose()
      }
      ...
      alert(`Error : 자료실 데이터 수정에 실패했습니다. `)
      handleClose()
    } finally{
      setLoadingBackdrop(false)
    }
