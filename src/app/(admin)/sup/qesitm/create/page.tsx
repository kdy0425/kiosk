...
const SrvyCreatePage = () => {
  ...
  const [srvyInfo, setSrvyInfo] = useState<SrvyInfoType>({
    srvyTtl: '',
    srvyGdMsgCn: '',
    srvyBgngYmd: getFormatToday(),
    srvyEndYmd: getDateRange('d', 30).endDate
  })
  ...
  const handleQParamChange = ( event: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement> ) => {
    const { name, value } = event.target;
  ...
  const handleItemParamChange = ( event: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement> ) => {
    const { name, value } = event.target;
  ...
  const handleQListAdd = () => {
    ...
    //   item.srvyQitemSn = QList.length > 0 ? QList[QList?.length - 1]?.srvyQitemSn + 1 : 1;
    // })
  ...
  const handleQlistRemove = () => {
    if (selectedQ > -1) {
      ...
      if (QList[selectedQ].isNew == false) {
        deleteQlist();
      }
  ...
  const saveSrvyInfo = async () => {
    try {
      ...
      let endpoint: string = `/fsm/sup/qesitm/createQesitm`;
      ...
      if (response && response.resultType == 'success' && response.data) {
        setCycle(Number(response.data.srvyCycl))
        return response.data.srvyCycl;
      } else {
        return false;
      }
    } catch (error) {
  ...
  // 설문 항목 저장
  const saveQList = async (type : string, srvyCycl:number) => {
    try {
      setLoadingBackdrop(true);
      ...
      // 회차정보를 획득하기 위해 첫 문항, 선택문항 저장 시 기본정보 저장
      if (srvyCycl == 0) {
        srvyCycl = await saveSrvyInfo()
      }
  ...
  // 선택 항목 저장
  const saveItemList = async (srvyCycl : number) => {
    try {
      setLoadingBackdrop(true);
      ...
      // 회차정보를 획득하기 위해 첫 문항 or 선택문항 저장 시 기본정보 저장
      if (srvyCycl == 0) {
        srvyCycl = await saveSrvyInfo()
      }
  ...
  //선택 항목 저장 전체
  const saveItemListAll = async (srvyCycl:number) => {
    try {
      let endpoint: string = `/fsm/sup/qesitm/updateArtcl`;
      let reqList: {
        srvyQitemSn: number;
      ...
      let requestItemData = QList.map(q => [...q.itemArr]).flat();

  // reqList에 모든 항목을 추가합니다.
    ...
    try {
      ...
      if (response && response.resultType === 'success' && response.data) {
        ...
        list.map(async (obj) => {
          ...
          obj.isNew = false; // 서버에서 받아온 값이라는 Flag, false인 값은 삭제 시 재조회 필요
    ...
    try {
      ...
      if (response && response.resultType === 'success' && response.data) {
        ...
        if (itemList && itemList.length > 0) {
          itemList.map((item: { isNew: boolean }) => item.isNew = false);
        }
  ...
  const deleteQlist = async () => {
    try {
      let endpoint = `/fsm/sup/qesitm/deleteQitem`
      ...
      if (response && response.resultType === 'success') {
        // 문항 하위 선택항목들 삭제
        const itemList = QList[selectedQ].itemArr;

        if (itemList && itemList.length > 0) {
          itemList.map(item => deleteItem(item.chcArtclSn));
        }
  ...
  const deleteItem = async (chcArtclSn: number) => {
    try {
      let endpoint = `/fsm/sup/qesitm/deleteArtcl`

      let body = {
        srvyCycl: cycle,
        srvyQitemSn: QList[selectedQ].srvyQitemSn,
        chcArtclSn: chcArtclSn
      }
  ...
  const saveAllContents = async () => {
    try {
      ...
      await saveQList("saveAll", srvyCycl);
      await saveItemListAll(srvyCycl);
    ...
    <PageContainer title="문항관리 신규등록" description="문항관리 신규등록">
      ...
      <Box component="form" onSubmit={() => { }} >
          ...
          <Table className="table table-bordered">
            ...
            <TableBody>
              <TableRow>
                ...
                <TableCell>
                  <CustomTextField id="srvyTtl" name='srvyTtl' value={srvyInfo.srvyTtl} onChange={handleSrvyInfoChange} fullWidth />
                </TableCell>
                <th className="td-head td-left" scope="row">
                  시작일자
                </th>
                <TableCell>
                  <CustomTextField id="srvyBgngYmd" name='srvyBgngYmd' value={srvyInfo.srvyBgngYmd} onChange={handleSrvyInfoChange} type='date' fullWidth inputProps={{min:getFormatToday()}}/>
                </TableCell>
                <th className="td-head td-left" scope="row">
                  종료일자
                </th>
                <TableCell>
                  <CustomTextField id="srvyEndYmd" name='srvyEndYmd' value={srvyInfo.srvyEndYmd} onChange={handleSrvyInfoChange} type='date' fullWidth inputProps={{min:getFormatToday()}}/>
                </TableCell>
              </TableRow>
              <TableRow>
                <th className="td-head td-left" scope="row">
                ...
                <TableCell colSpan={5}>
                <CustomFormLabel className="input-label-none" htmlFor="modal-srvyGdMsgCn">설문안내메시지</CustomFormLabel>
            ...
            <TableBody>
              {
                QList.map((item, index) => (
                  <TableRow onClick={(e: React.MouseEvent) => { e.stopPropagation(); handleQClick(index) }} selected={index == selectedQ} hover style={{ cursor: 'pointer' }}>
            ...
            <TableBody>
              {
                selectedQ > -1 && QList[selectedQ].itemArr.map((item, index) => (
                  <TableRow onClick={(e: React.MouseEvent) => { e.stopPropagation(); handleItemClick(index) }} selected={index == selectedItem} hover style={{ cursor: 'pointer' }}>
          ...
          <TableContainer sx={{ mr: 2 }}>
            <Box className='table-bottom-button-group'>
              ...
              <Box className=" button-right-align">
                ...
                <Button variant='contained' color='primary' disabled={!(Array.isArray(QList) && QList.length != 0)} onClick={() => saveQList("save", cycle)}>문항저장</Button>
              </Box>
            </Box>
            <Table className="table table-bordered" sx={{ minHeight: '100px' }}>
              <caption>문항 처리</caption>
              <TableBody>
                <TableRow>
                  <th>문항내용</th>
                  <TableCell>
                    <CustomFormLabel className="input-label-none" htmlFor="modal-qstnChcArtclCn">내용</CustomFormLabel>
                    <textarea className="MuiTextArea-custom"
                ...
                <TableRow>
                  <th>배점</th>
                  <TableCell>
                    <CustomTextField type='text' name='chcArtclScorScr' id='chcArtclScorScr' value={qParams.chcArtclScorScr} onChange={handleQParamChange} fullWidth />
                  </TableCell>
                </TableRow>
              </TableBody>
            </Table>
          ...
          <TableContainer sx={{ ml: 2 }}>
            <Box className='table-bottom-button-group'>
              ...
              <Box className=" button-right-align">
                ...
                <Button variant='contained' color='primary' disabled={selectedQ < 0 || QList[selectedQ].itemArr.length == 0} onClick={() => saveItemList(cycle)}>항목저장</Button>
              </Box>
            </Box>
            <Table className='table table-bordered' sx={{ minHeight: '100px' }}>
              <caption>문항 처리</caption>
              <TableBody>
                ...
                <TableRow>
                  <th>배점</th>
                  <TableCell>
                    <CustomTextField type='text' name='chcArtclScorScr' id='chcArtclScorScr1' value={itemParams.chcArtclScorScr} onChange={handleItemParamChange} fullWidth />
                  </TableCell>
                  <th>주관식</th>
                  <TableCell>
                  <CustomFormLabel className="input-label-none" htmlFor="sbjctvYn">주관식</CustomFormLabel>
                    <CommSelect
                      cdGroupNm={'786'}
        ...
        <Box className='table-bottom-button-group' style={{ marginTop: '50px' }}>
          <Box className=" button-right-align">
            <Button variant='contained' color='primary' onClick={() => saveAllContents()}>저장</Button>
