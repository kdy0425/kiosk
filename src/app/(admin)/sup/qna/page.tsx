...
export interface Row {
  ...
  leadCnCd?: string // 머릿글내용코드    // QnA 코드임.
...
const DataList = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/sup/qna?page=${params.page}&size=${params.size}` +
        ...
        // 관라자(운영자), 국토부 권한일때 조회 가능
        `${'&locgovUsrYn=' + ((isAdmin || isMinistry) ? 'Y' : 'N')}`+
  ...
  const fetchDetailData = async (row: Row) => {
    try {
      // 검색 조건에 맞는 endpoint 생성
      let endpoint: string = `/fsm/sup/qna/` + row?.bbscttSn
