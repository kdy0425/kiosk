...
export interface Row {
  ...
  leadCnCd?: string // 머릿글내용코드    // 민원처리 사례 코드임.
...
const DataList = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let endpoint: string =
        `/fsm/sup/cc?page=${params.page}&size=${params.size}` +
  ...
  const fetchDetailData = async (row: Row) => {
    try {
      // 검색 조건에 맞는 endpoint 생성
      let endpoint: string = `/fsm/sup/cc/` + row?.bbscttSn
  ...
  return (
    <PageContainer title="민원처리 사례" description="민원처리 사례">
      {/* breadcrumb */}
      <Breadcrumb title="민원처리 사례" items={BCrumb} />
      {/* end breadcrumb */}
