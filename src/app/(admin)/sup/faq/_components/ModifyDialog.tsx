...
export default function ModifyDialog(props: ModifyDialogProps) {
    const { title, size, open,selectedRow, isAdmin, isMinistry, handleDetailCloseModal,reloadFunc } = props;
    ...
    const deleteCode = async () => {
        ...
        const userConfirm = confirm("FAQ 데이터를 삭제하시겠습니까?");
        if(!userConfirm) return

        try{
            setLoadingBackdrop(true)

            let endpoint: string = `/fsm/sup/faq/deleteBoardDtl?`+
    ...
    const fetchFilesDown = async (files : CustomFile) => {
        ...
        try {
            ...
            let endpoint: string = `/fsm/sup/faq/file/`+files.bbscttSn+`/` +files.atchSn
    ...
    const fetchFilesDelete = async (files : CustomFile) :Promise<boolean> => {
        ...
        try {
            ...
            let endpoint: string = `/fsm/sup/faq/file/deleteBoardFile?` +
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const endpoint = `/fsm/sup/faq/updateBoardDtl`;
            ...
            if (response?.resultType === "success") {
                alert("FAQ 데이터가 수정되었습니다.");
                reloadFunc?.();
                handleClose();
            } else {
                console.error("Response fail:", response);
                alert(`FAQ 데이터 수정 응답이 성공이 아닙니다. (${response?.message || "Unknown Error"})`);
                reloadFunc?.();
                handleClose();
            }
        } catch (error) {
            console.error("Error during update:", error);
            alert(`Error : FAQ 데이터 수정에 실패했습니다. `);
            handleClose();
        } finally{
            setLoadingBackdrop(false)
        }
        ...
        >
            ...
            <Box className='table-bottom-button-group'>
                ...
                { isAdmin || isMinistry ? (
                    !isEditMode ?
                    <>
                        <Button variant="contained" onClick={handleEditToggle} color="primary">
                            수정
                        </Button>
                    </>
                    :
