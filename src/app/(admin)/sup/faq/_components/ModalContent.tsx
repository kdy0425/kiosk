...
const RegisterModalForm = (props: ModalFormProps) => {
  ...
  const createNotice = async () => {
    try {
      ...
      // 사용자 확인
      const userConfirm = confirm('FAQ 데이터를 등록하시겠습니까?');
      if (!userConfirm) {
        return;
      }

      const endpoint = '/fsm/sup/faq/createBoardDtl/';
      ...
      if (response?.resultType === 'success') {
        ...
        alert(`FAQ 데이터 등록 응답이 성공이 아닙니다. (${response?.message || 'Unknown Error'})`);
        handleClose();
        reloadFunc?.();
      }
    } catch (error) {
      ...
      alert(`Error : FAQ 데이터 등록에 실패했습니다. `)
      handleClose();
