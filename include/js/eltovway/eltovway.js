...
eltovway.Maps = class {
    ...
    constructor(param) {
        ...
        this.facilityContentsList = new Array();
        this.facilityRouteList = new Array();
    ...
    async loadData(kioskCode) {
        ...
        console.log('jsonContentsData', this.jsonContentsData)
        console.log('jsonRouteData', this.jsonRouteData)

        this.header = this.jsonContentsData.HEADER;
        console.log('headerdddddddddddddddddddddd',this.header)
        ...
        // storeList에서 CATE_CODE가 109일 때 구분
        this.facilityContentsList = this.storeContentsList.filter(fac => fac.CATE_CODE.text === "109");
        this.facilityRouteList = this.storeRouteList.filter(fac => fac.CATE_CODE.text === "109");
    ...
    loadMap() {
        ...
        let footerHtml = `
            <div class="floor_lnb" style="display: none;">
                <div class="depth1">
                    <nav id="divZone"> 
        ...
        for(let i = 0; i < _mapList.length; i++) {
            ...
            divCenter.style.transform = `scale(${this.defaultMapRate})`;
            ...
            const scale = this.defaultMapRate;
    ...
    onClickFloorbtn(floorCode) {
        ...
        allCenterDivs.forEach(div => {
            div.style.transform = " scale(" + this.defaultMapRate + ") translate(0px, 0px)";
        });
    ...
    loadPub() {
        ...
        for(let i = 0; i < this.facilityRouteList.length; i++) {
            let facility = this.facilityRouteList[i];

            let posX = parseInt(facility.STORE_FLOOR.pos_x);
            let posY = parseInt(facility.STORE_FLOOR.pos_y);

            let facilityEl = document.createElement("div");
            let facilityImgEl = document.createElement("img");
            facilityImgEl.classList.add('facilityImg');

            facilityEl.dataset.facilityCode = facility.CATE_CODE.sub_cate;
            facilityEl.dataset.facilityId = facility.id;
            facilityEl.dataset.floor = facility.STORE_FLOOR.text;
            facilityEl.dataset.area = facility.area_type;
            facilityEl.className = "facility_info";
            facilityEl.style.position = "absolute";
            facilityEl.style.left = posX + "px";
            facilityEl.style.top = posY + "px";
            facilityEl.style.transform = "translate(-50%, -50%)";

            facilityEl.dataset.midCate = facility.CATE_CODE.sub_cate;

            const facImgMap = {
                '10901': "./images/pubIcon/p_ico_toilet.png",
                '10903': "./images/pubIcon/p_ico_smoking.png",
                '10904': "./images/pubIcon/p_ico_baby.png",
                '': "./images/pubIcon/p_ico_aed.png"
            }

            // const altText = facility.CATE_CODE.sub_cate === '10901' ? '화장실' :
            //                 facility.CATE_CODE.sub_cate === '10902' ? '기도실' :
            //                 facility.CATE_CODE.sub_cate === '10903' ? '흡연실' :
            //                 facility.CATE_CODE.sub_cate === '10904' ? '유아휴게실' :
            //                 facility.CATE_CODE.sub_cate === '10905' ? 'AED' : '기타'; // alt 값 설정

            const facilityDesc = facility.STORE_DESC_KOR;
            const facilityName = facility.STORE_NAME_KOR;
                
            facilityImgEl.style.width = this.header.PUB_ICON_RESOLUTION.width + "px";
            facilityImgEl.style.height = this.header.PUB_ICON_RESOLUTION.height + "px";
            facilityImgEl.src = facImgMap[facility.CATE_CODE.sub_cate];
            // facilityImgEl.alt = `${altText} 아이콘`;
            facilityImgEl.alt = `${facilityName} 아이콘 : ${facilityDesc}`;

            facilityEl.style.display = 'none';

            facilityEl.appendChild(facilityImgEl);
            this.mapContainer.querySelector('.divMap[data-floor-code=' +  facility.STORE_FLOOR.text + ']').appendChild(facilityEl);
           
            // 매장 팝업
            facilityEl.addEventListener("click", () => {
                this.onClickInfoStoreById(facility.id,false);
            });
        }
        for(let i = 0; i < this.pubRouteList.length; i++) {
            ...
            pubImgEl.alt = `${this.pubContentsList.find(item => item.PUB_CODE == pub.PUB_CODE).PUB_NAME} 아이콘`;
        ...
        for(let i = 0; i < this.storeRouteList.length; i++) {
            ...
            if(storeContentsObj) {
                ...
                if(textCateList.includes(storeContentsObj.CATE_CODE.text)) { // 탑승구
                    ...
                    storeDiv.classList.add('img_bg');
                    imgBoxDiv.classList.add('img_box');
                    storeImg.classList.add("storeImg");

                    const storeDesc = storeContentsObj.STORE_DESC_KOR;
                    const storeName = storeContentsObj.STORE_NAME_KOR;
                    let storeCateCode = storeContentsObj.CATE_CODE.text
                    storeImg.src = storeContentsObj.STORE_LOGO_URL;
                    storeImg.alt = `${storeName} 아이콘 : ${storeDesc}`;
                    storeImg.onerror = () => {
                        // TODO 이미지 없을 시 대체 이미지 경로 수정
                        storeImg.src = `./images/category/ico_category_${storeCateCode}.png`;
                        // storeImg.alt = `카테고리 ${storeCateCode}번 아이콘`;
                        storeImg.alt = `${storeName} 아이콘 : ${storeDesc}`;
                    }

                    // storeDiv.style.display = 'none';
                    storeDiv.dataset.storeId = storeContentsObj.id;
                    storeDiv.dataset.bigCate = storeCateCode;
                    storeDiv.dataset.midCate = storeContentsObj.CATE_CODE.sub_cate;

                    imgBoxDiv.appendChild(storeImg);
                    storeDiv.appendChild(imgBoxDiv);
                    storeEl.appendChild(storeDiv);
    ...
    loadCurrent() {
        ...
        currentImgEl.src = "./images/currLocation-2.svg";
        currentImgEl.alt = "현재위치 아이콘";
    ...
    loadPubArea() {
        ...
        let displayFacList = ['10901', '10903', '10904'];
        ...
        // });
        let facList = this.facilityContentsList.filter(fac => displayFacList.includes(fac.CATE_CODE.sub_cate));
        facList.sort((a, b) => {
            return displayFacList.indexOf(a.CATE_CODE.sub_cate) - displayFacList.indexOf(b.CATE_CODE.sub_cate)
        });


        
        let pubAreaHtml = `
            <div class="floor_pub_area" style="display: none;">
                <button class="store_popup" id="storeListBtn">
                    <img src="./images/menu.svg" alt="리스트 보기 버튼">
                </button>
        ...
        const subCates = new Set();

        facList.forEach(fac => {
            const subCate = fac.CATE_CODE.sub_cate;
            
            if (!subCates.has(subCate)) {
        ...
        // HTML을 동적으로 생성
        let pubHtml = '';
        subCates.forEach(sub => {
            const imgSrc = facImgMap[sub];
            const bigCate = '109'; 
            const altText = sub === '10901' ? '화장실' :
                            sub === '10902' ? '기도실' :
                            sub === '10903' ? '흡연실' :
                            sub === '10904' ? '유아휴게실' :
                            sub === '10905' ? 'AED' : '기타'; // alt 값 설정

            pubHtml += `
                <li>
                    <button data-bigCate="${bigCate}" data-midCate="${sub}">
                        <img src="${imgSrc}" alt="${altText} 아이콘">
                    </button>
                </li>
            `;
        });
        ...
        document.getElementById("ulPub").querySelectorAll("button").forEach(button => {
            button.addEventListener("click",  () => {
                ...
                const pubCode = button.dataset.midcate;
    ...
    onClickPubIcon(floorCode, pubCode) {
        ...
        for(let i = 0; i < floorMapList.length; i++) {
            ...
            let floorPubCnt = this.facilityRouteList.filter(fac => fac.STORE_FLOOR.text == floorCode && fac.CATE_CODE.sub_cate === pubCode).length;
        ...
        // console.log(divMapEl.querySelectorAll(`.store_info > .storeImg[data-mid-cate="${pubCode}"]`), 'divMapEl.querySelectorAll(`.store_info > .storeImg[data-mid-cate="${pubCode}"]`)')
        divMapEl.querySelectorAll(`.facility_info[data-mid-cate="${pubCode}"]`).forEach((el) => {
            el.style.display = 'block';
            el.style.zIndex = 20;
        });

        this.tweenmaxPub = TweenMax.fromTo(divMapEl.querySelectorAll(`.facility_info[data-mid-cate="${pubCode}"]`), 0.5, { x: xx, y: yy, scale: 2 }, { x: xx, y: yy, scale: 1, yoyo: true, repeat: 8, onComplete: function () {
                divMapEl.querySelectorAll(`.facility_info[data-mid-cate="${pubCode}"]`).forEach((el) => {
    ...
    stopTweenmax() {
        if(this.tweenmaxPub) {
            document.body.querySelectorAll('.facility_info').forEach((el) => {
    ...
    onClickInfoStore(storeRouteObj, storeContents, isIframe) {
        ...
        // 매장 상세 비노출
        let displayNoneArr = ["109", "110"]; // 109 - 편의시설, 110 - 출입국시설
        ...
        const popupHtml = `
            <div id="divStorePopup" class="popup_store">
                <div class="popup_base">
                    <div class="popup_box">
                        <div id="btnClose" class="btn_close"><img src="./images/pop_bt_close.svg" alt="닫기 버튼"></div>
                        <div class="store_info">
                            ...
                            <div class="store_desc">
                                <ul>
                                    <li>
                                        <dl>
                                            <dt class="lang_code_names popup_time">${this.jsonLangData["index"]["POP_HOUR_TXT"][this.langCode.toLowerCase()]}</dt>
                                            <dd>${storeContents.STORE_SERVICETIME}</dd>
                                        </dl>
                                    </li>
                                    <li>
                                        <dl>
                                            <dt class="lang_code_names">${this.jsonLangData["index"]["POP_TEL_TXT"][this.langCode.toLowerCase()]}</dt>
                                            <dd>${storeContents.STORE_PHONE}</dd>
                                        </dl>
                                    </li>
                                    <li>
                                        <dl>
                                            <dt class="lang_code_names popup_items">${this.jsonLangData["index"]["POP_ITEM_TXT"][this.langCode.toLowerCase()]}</dt>
                            ...
                            <div class="store_location">
                                <h3 class="lang_code_names">${this.jsonLangData["index"]["POP_LOCATION_TXT"][this.langCode.toLowerCase()]}</h3>
                        ...
                        <div class="btn_box">
                            <button id="btnWayFind" class="btn_way">
                                <img src="./images/popup_ico_wayfinding.svg" alt="길안내 아이콘">
                                <span class="way_txt lang_code_names">${this.jsonLangData["index"]["POP_WAYFIND_WAYFIND"][this.langCode.toLowerCase()]}</span>
                            </button>
                            <button id="btnLocation" class="btn_wayfind">
                                <img src="./images/popup_ico_location.svg" alt="위치 아이콘">
                                <span class="way_txt lang_code_names">${this.jsonLangData["index"]["POP_WAYFIND_LOCATION"][this.langCode.toLowerCase()]}</span>
                            </button>
                        </div>
                    </div>
                </div>
        ...
        displayNoneArr.forEach((cate) => {
          if (cate.includes(storeContents.CATE_CODE.text)) {
        ...
        document.getElementById("btnLocation").addEventListener("click",  () => {
            ...
            if(document.querySelector(".currentImg")?.getAttribute("src") == "./images/departure.png") {
                ...
                document.querySelector(".currentImg").alt = "현재 위치 아이콘";
        ...
        if(this.header.KIOSK_SECT === 'T' && storeRouteObj.area_type !== 'T') {
            ...
            document.getElementById("btnWayFind").addEventListener("click", () => {
                ...
                document.querySelector(".currentImg").alt = "출발 아이콘";
                ...
                <div id="divWaytypePopup" class="popup_way">
                    <div class="popup_base">
                        ...
                        <div class="pop_waytype_box">
                            <h2>${this.jsonLangData["index"]["TRANSPORTATION"][this.langCode.toLowerCase()]}</h2>
                            <p class="title lang_code_names">${this.jsonLangData["index"]["POP_WAYTYPE_TITLE"][this.langCode.toLowerCase()]}</p>
                            <div class="btn_box">
                                <button id="btnEscalator" class="btn_esc" data-waytype-code="ESC">
                                    <img src="./images/popup_ico_escalator.svg" alt="에스컬레이터 아이콘">
                                    <div class="lang_code_names">${this.jsonLangData["index"]["POP_WAYTYPE_ESC"][this.langCode.toLowerCase()]}</div>
                                </button>
                                <button id="btnElevator" class="btn_ele" data-waytype-code="ELE">
                                    <img src="./images/popup_ico_elevator.svg" alt="엘레베이터 아이콘">
                                    <div class="lang_code_names">${this.jsonLangData["index"]["POP_WAYTYPE_ELE"][this.langCode.toLowerCase()]}</div>
                                </button>
                            </div>
                        </div>
                    </div>
    ...
    animationCallack() {
        ...
        if(this.wayfindParam.endObj.x == this.wayfindParam.endPos.x && this.wayfindParam.endObj.y == this.wayfindParam.endPos.y) {
            ...
            if(this.wayfindType === "PARK"){
                ...
                const storeCateCode = this.wayfindParam.endObj.storeObj.CATE_CODE;
                const storeSubCate = storeCateCode.sub_cate;
                const storeImgUrl = this.wayfindParam.endObj.storeObj.STORE_IMG_URL;

                let cateIcon = '';
                switch(storeCateCode.text) {
                    case '102': cateIcon = 'food'; break;
                    case '103': cateIcon = 'shopping'; break;
                    case '104': cateIcon = 'finance'; break;
                    case '105': cateIcon = 'Communication'; break;
                    case '106': cateIcon = 'travel'; break;
                    case '107': cateIcon = 'rest'; break;
                    case '108': cateIcon = 'subway'; break;
                    case '109': cateIcon = 'info'; break;
                    case '110': cateIcon = 'airplane'; break;
                    default : cateIcon = 'shopping'; break;
                }
                ...
                if(storeSubCate == "11001" || storeSubCate == "11002") { // 탑승구: 11001, 체크인카운터: 11002
                    ...
                    storeImg = document.createElement("img");
                    storeImg.src = storeImgUrl;
                    storeImg.alt = `${infoTitle} 로고 이미지`;
                    storeImg.onerror = () => {
                        // TODO 이미지 없을 시 대체 이미지 경로 수정
                        storeImg.src = `./images/default.png`;
                        storeImg.alt = `인천공항 로고 이미지`;
                    }
                    

                    infoTypeHtml = `
                    <div class="info_store_logo"></div>
                    <h3 class="info_title">
                        <img src="./images/pubIcon/p_ico_${cateIcon}.png" alt="카테고리 ${storeCateCode}번 아이콘">
                        ${infoTitle}
                    </h3>
                    `;
                }
            ...
            let wayfindResultInfoHtml = `
                <div id="wayEndInfoPopup">
                    <div class="info_box ${infoType}">
                        ...
                        <div class="btn_box">
                            <button class="replay" id="btnReWayfind">
                                <img src="./images/replay_w.svg" alt="다시보기 아이콘">
                                <span>${this.jsonLangData["index"]["POP_REPLAY"][this.langCode.toLowerCase()]}</span>
                            </button>
                            <button class="close">
                                <img src="./images/close_w.svg" alt="닫기 아이콘">
                                <span>${this.jsonLangData["index"]["POP_CLOSE"][this.langCode.toLowerCase()]}</span>
                            </button>
                        </div>
                    </div>
                </div>
            ...
            if(document.querySelector('.info_store_logo')) {
                document.querySelector('.info_store_logo').appendChild(storeImg);
            }
            document.getElementById("wayEndInfoPopup").style.left = "calc(50% - " + document.getElementById("wayEndInfoPopup").clientWidth/2 + "px)";

            // storeImgUrl.onerror = () => {
            //     // TODO 대체이미지
            //     storeImgUrl = `./images/default.png`;
            //     infoTitle = '인천공항';
            // }
            ...
            document.querySelectorAll("#popupCover, .close").forEach(el => {
                el.addEventListener("click", () => {
                    document.getElementById("wayEndInfoPopup")?.remove();
            ...
            document.getElementById("btnReWayfind").addEventListener("click", () => {
                this.isMove = true;
                const endObj = this.wayfindParam.endObj;
                const wayTypeCode = this.wayfindParam.moveType;
                const startFloorCode = this.header.KIOSK_FLOOR.text;
                const endFloorCode = endObj.floor;
    ...
    findMovePubInfo() {
        ...
        } else if (this.header.KIOSK_SECT === 'T' && this.header.KIOSK_FLOOR.text === 'F2'){
            _movePubCodeList = [];
        }
        ...
        if(_movePubCodeList.length > 0) {
            ...
            if(this.wayfindParam.endFloor == "F4" || this.wayfindParam.endFloor == "F2") {
            ...
            // 도착지가 환승호텔 또는 시작층이 2층일 경우
            if(this.wayfindParam.endObj.id === '00000007R4' || this.wayfindParam.startFloor === 'F2'){
                
                // 2, 3, 4층 전부 사용되는 이동수단 찾기
                const isExcludedPath = (floors) => {
                    const requiredFloors = ["F2", "F3", "F4"];
                    return requiredFloors.every(floor => floors.includes(floor));
                };
                ...
                // 이동수단 찾을 때  2, 3, 4층 전부 사용되는 이동수단 제외
                const findConnectingPubs = (currentFloor, nextFloor, pubsByFloor) => {
                    if (!pubsByFloor[currentFloor]) return [];
                    return pubsByFloor[currentFloor].filter(pub => {
                        const floors = pub.floor.split(',');
                        return floors.includes(nextFloor) && !isExcludedPath(floors);
                    });
                };
                ...
                const findShortestPath = (startFloor, endFloor, pubs) => {
                    ...
                    while (queue.length > 0) {
                        let [currentFloor, path] = ''

                        // KIOSK_CODE가 02EB인 경우 
                        if (this.header.KIOSK_CODE === '02EB') {
                            // `PUB_ID`가 "ELE_WT_2F3F_03"인 경로를 우선적으로 찾음
                            const indexWithTargetPub = queue.findIndex(([_, p]) => p.some(pub => pub.area === "ELE_WT_2F3F_03"));
                            
                            if (indexWithTargetPub !== -1) {
                                // `ELE_WT_2F3F_03`이 포함된 경로를 찾은 경우 해당 경로를 큐에서 꺼냄
                                [currentFloor, path] = queue.splice(indexWithTargetPub, 1)[0];
                            } else {
                                // 그렇지 않으면 일반적으로 큐의 첫 번째 경로를 처리
                                [currentFloor, path] = queue.pop();
                            }
                        } else {
                            // KIOSK_CODE가 '02EB'가 아닌 경우 일반적으로 첫 번째 경로를 처리
                            [currentFloor, path] = queue.shift();
                        }
                ...
                const groupByFloor = (data) => {
                    return data.reduce((acc, curr) => {
                        const floor = curr.PUB_FLOOR.text;
                ...
                // 최단 경로 찾기
                const findShortestPath = (startFloor, endFloor, pubs) => {
                    console.log(startFloor, endFloor, pubs, 'startFloor, endFloor, pubs')
                    const pubsByFloor = groupByFloor(pubs);
                    const queue = [[startFloor, []]]; // 현재 층과 경로를 담는 큐
                    const visited = new Set(); // 방문한 층을 기록하는 집합
                    console.log(this.wayfindParam)
                    while (queue.length > 0) {
                        let [currentFloor, path] = ''
                        if(this.header.KIOSK_CODE === '02EB'){
                            if(this.wayfindParam.moveType === 'ELE'){
                                [currentFloor, path] = queue.shift();
                            } else {
                                [currentFloor, path] = queue.pop();
                            }
                    ...
                    if(Math.abs(currentPoint.x - targetPoint.x) <= nearLen && Math.abs(currentPoint.y - targetPoint.y) <= nearLen){
                        ...
                        if(currentCnt >= wayfindList.length){
                            ...
                            if(this.wayfindParam.startFloorPub?.PUB_FLOOR.pos_x == this.wayfindParam.endPos.x && this.wayfindParam.startFloorPub?.PUB_FLOOR.pos_y == this.wayfindParam.endPos.y) {
                                            ...
                                            <div class="move_info">
                                                <div class="move_animation">
                                                    <div class="move_animation_box">
                                                        <img id="imgMoveType" alt="이동 수단 아이콘">
                                                    </div>
                                                </div>
                                                <div class="move_desc">
                                ...
                                document.getElementById("imgMoveType").alt= '이동 수단(엘레베이터, 에스컬레이터) 이동 애니메이션';
                                setTimeout(() => {
                                    document.getElementById('moveLoadingPopup')?.remove();
    ...
    // 초기 상점 아이콘 삭제
    storeIconRemove(type) {
        document.querySelectorAll('.store_info > .img_bg').forEach(el => {
            // el.style.display = 'none';
        })
        document.querySelectorAll('.facility_info').forEach(el => {
            // el.style.display = 'none';
        })
    }
